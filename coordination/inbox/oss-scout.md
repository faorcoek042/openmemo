...
B OPENMEMO_TLS=self-signed → https GET / 200、/api/health 正常、
                             明文打同端口连接失败(000)、私钥 0600、证书 644
C 单实例探测在 TLS 下正确识别已有实例（1s 内拒绝第二个实例）
   ★ 这条必须测：探测原本写死 http://，开 TLS 后会永远探测失败 →
     误判"端口上没有我们的实例" → 可能抢走用户的数据目录。已让协议跟着监听走。
D OPENMEMO_TLS_HOSTS=100.64.135.105 → 该 IP 确实进了证书 SAN
   （NAT 外部 IP 不在本机网卡上，我们猜不到，所以留这个显式口子）
```
...
⚠️  当前是**明文 HTTP + 非回环地址**（0.0.0.0:10000）。
   浏览器只把 HTTPS 与 localhost 视为安全上下文，因此在这个地址下：
     · 录音转文字（F3）不可用 —— navigator.mediaDevices 为 undefined
     · 多标签页选主降级 —— navigator.locks 为 undefined
   这不是浏览器旧，任何浏览器都一样。
...
- 证书有效期 825 天（超过会被部分浏览器直接拒），剩余不足 7 天自动重新生成 ——
...
目标非空但**不是**我们的
  → 409 TARGET_NOT_EMPTY，**点名里面有什么**：
    「新位置里已经有别的内容（例如：我的论文.docx），而且它不是 OpenMemo 数据目录。
      为避免覆盖你自己的文件，这里不做搬运……原数据完好，未做任何改动。」
...
现在这种情况回 `NOT_A_DATA_DIR` 并说清楚。

判据抽成 `looksLikeDataDir()` 并测了 5 条（含**0 字节的 openmemo.db 不算数据目录** ——
...
lint 干净；move 18/18、roundtrip 16/16、upload 17/17。
...
然后无论哪种，都把最终 dataDir **显式写进 argv**（先剔除旧的 `--data-dir` 再追加）。
于是子进程走的是"显式旗标"这条唯一优先级，与正常启动完全一致，
**也不再受"重启这一刻指针恰好指向哪里"的影响**。
...
迁移到 /tmp/t110-new  → dataDir 变为 /tmp/t110-new                      ✅ 该切的切了
再点一次普通重启      → 仍停在 /tmp/t110-new                            ✅ 不回退
```
...
收尾：测试期间动过全局指针，**已还原为 `/root/data-memo`**；测试实例与临时目录全部清理。
lint 干净；move 18/18、restart-datadir 3/3、roundtrip 16/16；demo 全程只读未重启。
...
- **CSRF 没有 token 就失去意义**：它防的是"攻击页借用你已有的凭据"。
  ...
  但在无鉴权前提下它本来就能，CSRF 不改变这个结论。
- **Host/Origin 保留**：零成本，且挡的是 DNS rebinding 这种**与凭据无关**的真实威胁。
  这条与你的倾向一致。
...
- 判别方式：`GET /api/health` 增加 `auth: 'none' | 'token'` 字段（**待实现**，
