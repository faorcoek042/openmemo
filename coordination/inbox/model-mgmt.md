...
理由写进代码注释：_"上游 API 说的" 与 "我们下载了每一个字节自己算的" 是两种不同强度的证据 ——
前者信任上游没被攻破，后者只信任字节本身。合并成一个哈希串会高估可信度。_
...
## T-109 真浏览器取证：设置页两处不一致 + F4 导图可用性

⚠️ **前置说明**：demo（`:10000`，dataDir `/root/data-memo`）需要 token，我**没有去翻它的凭据文件**
（上次那次拦截是对的）。改用自建实例复现 —— 这两处的文案与控件形态**由代码决定、与数据无关**，
所以结论可直接对应；唯一没验到的是"用户真实 DeepSeek 产物能否渲染"，见末尾。
为忠实复现用户处境，我在自己实例上配了在线 provider（DeepSeek，假 key）并设为默认。

### ① 两处的控件逐条清点（**先打印候选元素再断言**）

**「AI 模型」区块** —— 服务商**不是下拉，是卡片列表**：

    顶部        当前生效: DeepSeek · deepseek-chat
    已配置卡片  DeepSeek（未设置 Key · 使用中）
                Ollama（本地）  徽标「本地，无需 Key」  按钮「设为默认 / 编辑」
    添加预设    + OpenAI    + Anthropic    + LM Studio（本地）
    展开「编辑」后的字段：
      接口地址（Base URL）  = https://api.deepseek.com/v1   （自由文本）
      模型名               = deepseek-chat                 （自由文本）★
      API Key              = （空，占位 sk-…）

**「按用途分别配置模型」区块** —— 三组（对话 / 摘要+思维导图 / 翻译），每组两个控件：

    服务商  = 真下拉，选项恰好 3 个：  继承全局 | DeepSeek | Ollama（本地）
    模型    = 自由文本输入，placeholder 显示继承值 deepseek-chat        ★
    脚注    实际生效：DeepSeek · deepseek-chat

**不一致点（用户问的"为什么不是统一的"），共 4 条：**

1. **同一个概念两个标签**：`settings.model` = 「**模型名**」 vs `settings.purposes.model` = 「**模型**」。
2. **服务商控件形态不同**：上面是卡片 + 「设为默认」，下面是 `<select>`。
3. **★ 两处的"模型"都没有下拉** —— 都是自由文本，只能手打。
   所以严格说「两处的模型下拉」**一个都不存在**，这可能正是用户觉得别扭的根源：
   服务商能选，模型却要自己敲，而且敲错了没有任何校验。
4. **层级不同**：上面的「模型名」属于某个 provider 卡片（改它等于改这个服务商的默认模型）；
   下面的「模型」是按用途的独立覆盖字段。名字几乎一样、作用域不一样。

### ② 「离线」字样的出处：**整个设置页没有「离线」，也没有「Offline」**

我在中英两种语言下都搜过页面正文，`离线` / `Offline` **零出现**。
页面上有的是「**本地**」，共 4 处，全部与 Ollama / LM Studio 绑定：

    1. 区块说明   「…云服务填 API Key；本地服务（Ollama / LM Studio）不需要 Key。」
    2. 卡片徽标   Ollama（本地）→「本地，无需 Key」
    3. 明文警告   「…若不接受，请改用本地模型（无需 API Key）。」
    4. 预设按钮   「+ LM Studio（本地）」

**我的判断（标注为推测，未坐实）**：用户的 key 没存进库（你说的 ④），
此时「当前生效」会显示 `未生效 —— 保存一个服务商即可启用（摘要/导图需要它）`，
而列表里最显眼的就是这几处「本地」字样 —— 用户很可能是**由此推断"现在是离线模式"**，
而不是看到了某个写着「离线」的控件。**要坐实需要他的截图或他那台机器的状态。**

### ③ F4 导图在网页上：**能看、能拖、能导出，但存不下来**

    渲染        ✅ 7 个中文节点全部渲染，mind-elixir 挂载，容器 .map-container
    拖拽重挂父   ✅ 把「修辞手法：对偶」拖到「演讲背景」上，节点从 (1012,566) → (1170,469)
    SVG 导出    ✅ 「JFK 就职演说片段.svg」6,814 B，含 JFK/公民责任/不要问国家/修辞手法，合法 XML
    PNG 导出    ✅ 70,275 B
    保存        ❌ 拖完**零写请求**

**保存为什么断**（这条可以立刻修）：

    MindmapPage.tsx:25   if (!MINDMAP_SAVE_SUPPORTED) return;
    api.ts               export const MINDMAP_SAVE_SUPPORTED = false;
    注释理由              「服务端还没有保存端点：不对着不存在的路由发请求。」

**但 daemon 的 PATCH 端点早就落地了** —— `content.ts:329`
（`PATCH /api/notes/:uid/mindmap`，带 `validate(doc)` 防环/悬空引用 + `revision` 乐观锁）。
**是前端这个常量没跟着翻回来。** 翻开它编辑就能存。
这是"声明与实现脱节"的又一例，但方向相反：**这次是后端做好了、前端还关着**。

另：**笔记详情页的「Mind map」tab 是只读的** —— `NoteDetailPage.tsx:175` 渲染
`<MindmapView doc={...} />` **没传 `onChange`**，所以在 tab 里怎么拖都不会保存；
要编辑得进专用页 `/notes/:uid/mindmap`。这个差异界面上没有任何提示，
用户会以为"在 tab 里改了"。建议 tab 里要么标只读、要么给「去编辑」入口。

### 我自己犯的一个错（已纠正，记下来）

第一次测拖拽用的是笔记详情页的 tab，且拿 `[class*="mind-elixir"]` 读属性 ——
选中的是 **`mind-elixir-ghost`（拖拽残影容器）**，得出「`draggable=false`、拖不动」的结论。
换到专用页、改用 `me-tpc` 精确选择后，拖拽是**好的**。
**又是"先质疑测试"救回来的一次误报** —— 差点把只读 tab 报成渲染器坏了。

### 截图

    /tmp/shots/t109-zh-settings.png     两个区块全景（中文，DeepSeek 编辑面板展开）
    /tmp/shots/t109-settings-full.png   英文版全景
    /tmp/shots/t109-mindmap.png         导图渲染
    /tmp/shots/t109-mindmap-page.png    专用页拖拽后
    /tmp/mmdl/JFK 就职演说片段.svg / .png   实际导出物

### 边界（不粉饰）

导图用的是我造的 fixture（4 个中文一级节点 + 3 层结构），**不是用户真实的 DeepSeek 产物**。
渲染 / 拖拽 / 导出这三条与数据来源无关，结论成立；
但「真实 LLM 产物（含可能的超长文本、特殊字符、深层嵌套）能否渲染」**我没验到**，
需要 demo 的 token 或把那条笔记的 doc 导出给我。
