...
| `lint-workflows.mjs`        | 1,161 | **89 静态**（扫描器报 ≥90，见下注）/ 1,978 运行时 | `must()` | 无 | ✅ `selftest-lintwf-coverage` + `lintwf-coverage`（手动） |
...
- `lint-workflows.mjs` 的「2,000 条」是**运行时**条数，与其余各行的**静态调用点**
  不是一个量纲，已分开写。
- 🔴 **`count-verdict-sites.mjs` 把助手自己的定义也数进去了 —— 于是那个 `≥N`
  不是一个下界。** `[我核过 @ `05b1afe`，第二版扫全量之后仍然成立]`
  它给 `lint-workflows.mjs` 报 **≥90**，而真正的调用点是 **89**：多出来的那一个是
  `function must(cond, msg)` **本身**（正则 `(^|[^A-Za-z0-9_.$])must\s*\(` 会匹配
  `function must(` 里那个空格 + `must(`）。

  ⚠️ 要紧的不是"差 1"，是**方向**：第二版刻意把输出改成 `≥N` 并声明"只用来发现 0"，
  而 `≥90 > 89` 说明**它声称的下界并不成立**。一个偏高的下界比一个偏低的更难发现 ——
  它永远不会触发那条"数出 0 就出声"的防线。

  **这不是 lint-workflows 独有的**：凡是助手定义在自己文件里的脚本都多数 1，
  `ok`+`judge` / `ok`+`fail` 这种两个助手的多数 2。第二版扫的是**全量 96 个**，
  所以偏差面比第一版那张 11 行的表大得多，我没有逐个量。
  修法一行：数完再减掉 `function\s+<name>\s*\(` / `const <name> =` 的个数。
