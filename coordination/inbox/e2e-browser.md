...
## [2026-08-16] `B1 的证伪能力` 连红三夜 —— **不是偶发，也不是 win32 时序竞态**

### 一、定性：`boom` 与「没有变红」**没有关系**

同一份日志里的两行 `✘` 相隔 32 秒、分属两节，此前被读成了一件事：

- `✘ 未捕获异常：boom` 在 **2b 节**（「测速**失败时**用户看得懂吗」）。
  `boom` 是这条腿**自己注入**的 500 响应体（`e2e-browser-audit.mjs` 里
  `error: { code: 'PROBE_FAILED', message: 'boom', … }`），
  前端 `void mutateAsync()` 漏出未捕获拒绝 —— 那一节的断言 **B5 是 `✔`**。
- `✘ [变异] B1 的证伪能力 …` 在 **4 节**，另起一个 `/models` 页、另一次点击。

所以既不是「抛出没被判红」，也不是「注入机制炸了导致 B1 没跑」——
**B1 那一轮跑到了、判了、判成了"活着"。**

### 二、成因：判据量的是「窗里发生过什么」，不是「这一次点击导致了什么」

| run | 平台 | 变异体 | 窗内观测 | 判决 |
| --- | --- | --- | --- | --- |
| 31736237514 | win32 | 装上了 | DOM 自己变了、`/api` **0 条** | **MUT-BAD** |
| 31833084492 | win32 | 装上了 | DOM 自己变了、`/api` **0 条** | **MUT-BAD** |
| 31902320145 | win32 | 装上了 | DOM 自己变了、`/api` **0 条** | **MUT-BAD** |
| 31902320145 | darwin | 装上了 | DOM 自己变了、`/api` 4 条（catalog / storage / jobs ×2） | **MUT-BAD** |
| 31902320145 | linux | 装上了 | 全静止 | MUT-OK（**只是恰好静止**） |

darwin 那 4 条**没有一条**是点击导致的：catalog / storage 是首屏查询迟到落地，
`/api/jobs` 是任务列表 5 秒兜底轮询（`apps/web/src/lib/api/jobs.ts` 的 `JOBS_POLL_MS`）。
而这条腿在 2b / 3b 节**自己起过真实安装**，到 4 节时后台正忙 —— 所以「页面静止」
这个隐含前提在越来越多的轮次里不成立。三个平台的分歧从头到尾没有一格是产品差异。

同一个缺口在**绿的那些轮**里也一直在，三个平台逐字相同：
...
### 三、修法（`fix/b1-falsification`）

判据抽进 `scripts/ci/e2e-browser-assertions.mjs`（此前留在 2200 行的顶层脚本里，
`import` 不进来 ⇒ 永远喂不进输入），加三条、各有反向证明：

1. **①空转闸**：`clicked !== true` ⇒ 「有反应」必须是 `false`。
2. **空转对照窗**：点之前先量同样长的一段、什么都不点；那一段里页面自己发的请求、
   自己改的 DOM，判据不认。
3. **该发生的那件事**：点「立即测速」必须发出 `POST /api/models/sources/probe`。
   钉的是**产品契约**（`packages/shared/src/api.ts` 的 `ENDPOINTS.probeSources`，
   有契约漂移守卫），不是变异体的机制 —— 换一种弄死按钮的办法照样红。

外加：B1 基线轮与变异轮的判决改成**同一段代码**（`judgeProbeClick()`，此前两份手抄、
变异轮那份是基线那份的子集）；变异轮的**前提**（按钮在不在、点不点得到）从 `ok()`
改成 `undecided()` ⇒ MUT-UNKNOWN —— 否则「腿炸了」会落进 MUT-OK，
正是 v0.7.3 抓到的反方向假绿。

证明在 `scripts/ci/selftest-e2e-browser.mjs`（已挂进 `pnpm test:ci-scripts`，几十毫秒）：
三夜那四格的**原始数字**逐格照抄进用例，两条修法**逐条抽掉**都必须重新判"活"。

### 四、⚠️ 没修的（不假装已修）

- **第 3 节横扫那 40 个匿名按钮仍然只有老四选一**（少了 ①空转闸之外的两条）。
  给每个按钮加对照窗 = 每平台多约 60 秒墙钟，而且会把 `B3` 的判据一次性收紧
  40 处 —— 那是另一次改动，需要单独一轮 CI 观察，不塞进这一轮。
  **同形缺口在那一格里还在：页面自己在动时，一个死按钮可能被横扫判成活的。**
- `mutation()` 仍然把**任何**抛出记成 `MUT-OK`（只有 `Undecided` 例外）。
  Playwright 超时、页面崩了、断言实现自己抛，都还会被读成「如期变红」。
  这一轮只把 B1 那条的前提移出了这个坑，**没有动 `mutation()` 本身的结构**。
