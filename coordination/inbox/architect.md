...
3. **M-4 的「已保留（无对应更新）」徽标**（`flags & CONFIRMED`）。
   ...
   悬浮提示把因果说全了：_"重跑按时间轴对齐、不按段落序号 —— 两遍模型的断句本来就不同。
   这一段是你改过的，重跑里没有找到对应位置，因此按原样保留，不会被覆盖也不会被删除。"_
   不说清楚，用户会以为重跑漏了这一段。
...
## T-100 配置保存断在哪一步（2026-08-03）

### 四步收窄的结论

用**隔离实例**（`:17931` / dataDir `/tmp/om-t100` / `setsid`）打了完整往返：

| 步骤               | 结论                    | 证据                                                                                |
| ------------------ | ----------------------- | ----------------------------------------------------------------------------------- |
| 请求形状对不对     | ✅ **对**               | 裸 map PATCH → **200 且落库**（settings 3 行）                                      |
| 有没有发出去       | ✅ 发了                 | —                                                                                   |
| 服务端接不接受     | ⚠️ **缺 CSRF 头 → 403** | 同一请求去掉 `X-OpenMemo-CSRF` → **403**                                            |
| 失败有没有告诉用户 | ❌ **完全没有**         | `LlmSettingsSection` 里三个写 mutation 的错误**一个都没渲染**，且表单**无条件收起** |

**断点在第 4 步，而且是我的锅。** 用户拿到的唯一信号是"表单关了"——
他会合理地理解成"保存成功了"，然后刷新发现什么都没有。

⚠️ 一个必须澄清的点：**`{"settings":{...}}` 那个双包一层是探针本身的形状问题，不是前端发的。**
daemon 的 PATCH 契约收的就是**裸 map**，而 `usePatchSettingsMutation` 发的正是裸 map（实测 200）。
（daemon 那边把 `settings` 当成合法键存下来仍值得收紧，但**前端这条没有对不上**。）

### 修了什么

1. **失败可见**：`patch` / `setSecret` / `delSecret` 三个 mutation 的错误全部渲染成 `ErrorBlock`。
2. **失败不收表单**：改用 `mutateAsync` + `Promise.all`，**全部成功**才 `setEditing(null)`；
   失败时表单保持打开，**用户刚填的 key 不会丢**（这点比报错本身更实际）。
3. **成功有明确信号**：显示「✓ 已保存到本地（刷新页面后仍在）」——
   刻意把"刷新后仍在"写进文案，因为那正是用户被骗过的地方。
4. 保存中禁用按钮 + 显示"保存中…"。

**关于「写操作永不静默回落 mock」：它是生效的**（`client.ts` 确实把 403 抛出来了）。
这次的教训是 —— **规则生效 ≠ 用户看得见。抛出来的错误得有人接住并画出来，否则等于没抛。**
这和我之前那条"写了报告 ≠ 修了问题"是同一个形状。

### 完整往返实测（含刷新）

```
PATCH 裸 map          -> 200
PATCH 无 CSRF 头      -> 403          ← 静默失败的真凶
PUT  /api/secrets/... -> 200
--- 模拟刷新（同 cookie 重新 GET）---
GET /api/settings     -> 200  llm.defaultProviderId = deepseek
GET /api/secrets      -> 200  masked = sk-t…UMMY   ← 掩码还在
磁盘：settings 3 行 · secrets.json 权限 -rw-------
```

**刷新后 key 的掩码仍在。** 收尾按规矩：先确认 `/proc/<pid>/environ` 的
`OPENMEMO_DATA_DIR=/tmp/om-t100` 再 kill 自己那一个 pid；**demo `:10000` 复查仍 200，未受影响**。

### ⚠️ 另一个坑（不在我域内，请派 `oss-scout`）

**`POST /api/auth/session` 带着有效 cookie、但没有 Bearer → 401**（实测）。

后果：`#t=` fragment 取一次就从 URL 抹掉、且只存在内存里。
用户**刷新页面或开新标签**后，前端拿不到 token 去重新握手 ——
`connect.ts` 的 401 自愈路径**没有 token 可用，必然失败**。
此时若 cookie 仍有效，读还能用；一旦 daemon 重启（cookie 失效），
用户就**彻底卡死在"未认证"**，唯一出路是回去找那条带 `#t=` 的原始 URL。
这正好解释 T-074 那条「很多地方报『未认证，请重新打开应用』」。

建议：`POST /api/auth/session` 在**已有有效 cookie 时**直接续签并回一个新的 csrf
（不需要 Bearer）。这样刷新/新标签都能自愈，且不放宽任何信任边界 —— cookie 本来就是凭证。

### 验证

`tsc` 0 · `eslint apps/web` 0 · `vite build` ✓ ·
测试 **86 条 / 84 pass / 0 fail / 2 skip**（新增 3 条：403 要可见且不收表单 ·
成功要有明确信号且请求体是裸 map · 端点不存在时绝不显示已保存）。

### 顺带：TLS 撤销后的文案调整

按你说的**弱化了"启用 HTTPS"**：横幅与录音页现在只给 **localhost** 一个可点的动作，
https 只在说明文字里作为事实提一句，不再当按钮推销。
非安全上下文的**检测与告知照旧保留** —— 那是告知事实（此地址下录音不可用），不是安全策略。

### 诚实声明

- **没碰 demo**，只对 `:10000` 的公开 `/api/health` 做了一次收尾复查（200）。
- **没有在真浏览器里走一遍这个 UI** —— 证据是隔离实例的 HTTP 往返 + 3 条组件测试。
  真浏览器里最值得复验的是：**保存失败时错误条幅是否真的出现在用户视线内**
  （我把它放在区块顶部 `MockNotice` 之下，长表单下滚动时可能需要往上看）。

---

## T-103 CSRF 令牌不再静默丢失（2026-08-03）

### ① `sessionStorage` 为什么"不可用"——**它多半根本没坏**

把排查引向"隐私模式/存储被拦"是**错的方向**，我先把这条纠正掉：

- `sessionStorage` **不是 secure-context-gated 的**。`http://<IP>` 下它**照常可用** ——
  挡住它的是无痕模式、"阻止所有站点数据"这类设置，与 http/https 无关。
- 真正的机制是**存放范围不对称**：
  **cookie 是 per-origin（跨标签共享），而 CSRF 令牌是 per-tab（`sessionStorage`）。**

于是**开第二个标签页**（复制 URL、或从别处点进来，都不带 `#t=`）就完美复现用户现象：
新标签里 cookie 有效 → **读全通、界面一切正常**；没有令牌 → **写全 403、库里 0 行**。
不需要任何"存储故障"，这是 `sessionStorage` 的**正常语义**。

隔离实例实测复现（`:17932`）：

```
GET   /api/settings  (读)          -> 200
PATCH /api/settings  无 CSRF (写)  -> 403   ← 事故现象
```

叠加上一轮实测的 `POST /api/auth/session` 持有效 cookie 但无 Bearer → **401**，
新标签**连补救的机会都没有**：拿不到 token 就重新握手不了。

### ② 已改为**内存存令牌**

`csrfToken` 模块级变量是**权威副本**，`setCsrf()` **先写内存**（这一步不会失败），
`sessionStorage` 降级为**可选加速**（刷新后省一次握手），存不进去只少一点便利，
**不再影响正确性**。新增 `clearCsrf()` / `hasCsrf()`（**不回显令牌本身**）。

**那句注释是这次事故的核心**：「降级为无 CSRF 头，**由 Origin 校验兜底**」——
兜底是我方**单方面想象**出来的，服务端是硬拒。
教训我记在这儿：**跨端"降级"必须以对端确实存在的行为为依据，否则它不是降级，是静默失败。**
而且两个错缺一不可 —— ①假设了不存在的兜底 ②降级是静默的。

### ③ 静默降级已消灭 + 403 自愈

- **写请求前若令牌不在手上，先补一次握手**（`if (isWrite && !hasCsrf()) await reHandshake()`）。
  补不上**也照发** —— 让服务端给出**真实拒绝**，而不是我们在本地判定失败
  （本地判定会掩盖"其实服务端允许"的情况，正如你裁定的同源兜底）。
- **CSRF 403 自愈**：与 401 同一形状，重新握手一次再重试。
  只认 `code === 'CSRF_FAILED'` 或 `remediation.action === 'reauth'` ——
  不能把 `FORBIDDEN_ORIGIN` 那类**重握手也没用**的 403 也拖进来空转。
  已与 `oss-scout` 加的 `retryable:true` + `remediation{action:'reauth'}` 对齐。
- 上一轮的保存反馈仍在：失败渲染 `ErrorBlock`、**表单不收起**（用户填的 key 不丢）、
  成功显示「✓ 已保存到本地（刷新页面后仍在）」。

### ④ 你裁定的同源兜底**已经生效**（我实测到了）

同一隔离实例上 `PATCH` **无 CSRF 头 → 200** ——
说明 `oss-scout` 那条已经进了当前 dist。按你的定位，它是**兜底**：
我这侧仍然优先把令牌拿对，不让兜底成为常态。

### 顺带

`lib/secure-context.ts` 加了 `isSessionStorageAvailable()` 并纳入能力清单，
但**如实标注它不是 secure-context 的锅** —— 放进同一张清单只是为了排查时只看一个地方，
不是把它归因给非安全上下文。归错因比不报更浪费时间（`multiTab` 那次就是）。

### 验证

`tsc` 0 · `eslint apps/web` 0 · `vite build` ✓ ·
测试 **90 条 / 88 pass / 0 fail / 2 skip**（新增 4 条：存储抛异常也不丢令牌 ·
`clearCsrf` 内存与缓存都清 · 403 自愈且**确实重新握手过**再重试 ·
`sessionStorage` 归类不许栽给非安全上下文）。

隔离实例往返（`:17932`，`setsid`，收尾前核对 `/proc/<pid>/environ` 的 dataDir 再杀自己那一个 pid）：
带 CSRF 写 → 200 → **刷新回读仍在**。**demo `:10000` 复查 200，全程未受影响。**

### 诚实声明

- **没碰 demo**，只对公开 `/api/health` 做了两次收尾复查。
- **没在真浏览器里开第二个标签页验证过** —— 复现与修复的证据是隔离实例的 HTTP 往返
  - 4 条组件测试。真浏览器里值得验的正是这条：**开第二个标签页 → 直接保存 → 应当成功**。
- 用户说等环境稳定再设 key。就我这侧而言链路已通；
  但**新标签/刷新后的自愈完全依赖 `POST /api/auth/session` 能用 cookie 续签** ——
  那条目前仍是 401（上一轮实测），已在上一份报告里建议给 `oss-scout`。
  在它落地之前，兜底扛着，但根因未除。

---

## T-107 顶部横幅合并（2026-08-03）

### 结果：**7~8 行 → 1 行（折叠态）**，全就绪时 **0 行**

按 demo 当前真实 health（`/root/data-memo`，`tokenizer=simple libsimple=true sqliteVec=true`，
`pipeline.missing=[]`，`restartRequired=false`）：**后端相关条目 0 条**。
用户从 `http://<IP>` 访问再 +1（安全上下文，**三项合并为一条**）。
...
```
当前地址下有 3 项浏览器能力不可用（含录音）                    [用 127.0.0.1 打开]
  浏览器只在 https 或 localhost 下开放麦克风、剪贴板与标签页选主。
  改用 http://127.0.0.1 打开即可恢复（前提是你就在运行 OpenMemo 的这台机器上）。
  这与浏览器品牌无关，换浏览器不会有帮助；其余功能不受影响。
```

（以上是在组件宿主里真实渲染后 dump 的文本，不是我手写的示意。）

### ① 可修复的配置状态不再像产品缺陷

新 `components/common/ReadinessBanner.tsx` 合并三处来源，**每项带可点的动作**
（去装分词器 / 去装模型 / 立即重启 / 用 127.0.0.1 打开），
数据源仍是 `/api/health` 轮询 + 运行时特性检测，**装好后自行消失**。
「装好了待重启」仍优先于「未启用」—— 否则会把已装好的用户送回去再装一遍。

### ② 安全上下文从"说三遍"变成"说一遍"

`multiTab` 顶层横幅与 `SecureContextBanner` **都已删除**（文件一并删掉，不留两份真相），
`banner.multiTab` 这个 i18n 键也删了 —— 留着它两处文案迟早各自漂移。
有一条测试断言"同一件事只出现 1 行"（即便 SSE 层同时报了多标签降级）。

⚠️ 保留了一个**真降级**分支：安全上下文正常、却仍没有 Web Locks（很旧的浏览器）时
才单独提示。合并不等于把信息弄丢。

### ③ 默认状态就是"什么都不说"

`items.length === 0` 时**渲染 `null`**，一个像素都不占（有用例钉住）。
折叠态只给结论，**原因和办法一个字都不在首屏**，点开才出现。

**我自己的教训**：上一轮我写过"给可点的动作而不是让用户猜"，
但那一版把结论、原因、办法七行全铺在首屏 ——
**七行文字不是动作，是墙。可操作性和信息量是两回事：前者要的是一个按钮，不是一段说明。**

### 与 `ui-polish` 的边界（请转达）

横幅属 app 层，归我。我这侧定的**结构契约**如下，视觉细节（配色/间距/图标）他随意改，
但请**别改这三条**，它们是这次反馈的直接产物：

1. 全就绪 → 渲染 `null`；
2. 折叠态 **只占一行**，且只放结论；
3. 每项必须带**可点的动作**，不留纯陈述项。

复用的 testid：`readiness-toggle` / `readiness-details` / `readiness-action-<key>`。

### 验证

`tsc` 0 · `eslint apps/web` 0 · `vite build` ✓ ·
测试 **92 条 / 90 pass / 0 fail / 2 skip**（本轮新增/改写 5 条：全就绪不渲染 ·
默认折叠且原因不上首屏 · 同一件事只一行 · 每项都有修复动作 · 归因不许说"浏览器不支持"）。

### 诚实声明

- **没碰 demo**，只对公开 `/api/health` 读了一次以确认真实条目数。
- **没在真浏览器里看过合并后的样子** —— 上面那段是组件宿主渲染后的**文本** dump，
  能证明层级与措辞，**不能证明视觉观感**。真机复验请 `ui-polish` 顺手带一眼。
- 用户当前从 `http://<IP>` 访问，所以他刷新后**仍会看到 1 行**（不是 0 行）。
  这一行是**事实**（此地址下确实录不了音），不是误报 —— 换成 `127.0.0.1` 后会自己消失。

---

## T-108 LLM 配置：键对齐 + 在线优先 + 单一数据源（2026-08-03）

### ③ 两个键已写上（这是主因）

`onSave` 之前只写 `providers` + `baseUrl`，`defaultModelId` 还得"已经是默认才写"，
**`defaultProviderId` 一次都没写过** —— 而它是 daemon 唯一认的键。
用户走的正是"加 provider → 填 key → 保存"，从不点「设为默认」，所以永远缺键。

修法不是补一行，是**把两个入口收敛到一个生成器** `buildLlmSettingsPatch()`：
保存与「设为默认」共用它。**两个入口各拼各的一组键，就一定会有一个先忘** —— 上次就是这么漏的。
另加：**没有默认 provider 时，保存哪个就让哪个生效**（用户的心智是"我配好了就该能用"）。

### 端到端实测（隔离实例 `:17933`，没碰 demo）

```
① 复现用户形状（只有 providers + baseUrl）
   resolveConfiguredProvider() => ❌ undefined  = LLM_NOT_CONFIGURED
② 应用 UI 新 patch（四个键）后触发 F4
   job state = failed
   error     = auth failed (401) from deepseek: "Your api key: ****real is invalid"
```

**第二步的 401 就是证据**：任务已经走到真的去调 DeepSeek 了 ——
provider **解析成功**，只是我用的是假 key（`sk-DUMMY-not-real`）。换成真 key 即通。

⚠️ **中途我差点误判**：第一次跑拿到 `blocked_code = NO_TRANSCRIPT` 就想收工，
查了 `mindmap.ts` 才发现 `NO_TRANSCRIPT`(L77) 在 `LLM_NOT_CONFIGURED`(L109) **之前**——
那次根本没走到 LLM 检查。补了一条转写稿才真正验到。**"没报那个错"不等于"那步过了"。**

### ① 在线优先（ADR-016）

- 预设从 5 家扩到 **11 家**：在线 9（DeepSeek / OpenAI / Anthropic / Gemini / Kimi /
  智谱 / 百炼 / 硅基流动 / OpenRouter）+ 本地 2（Ollama / LM Studio）。
- **顺序与分组都改了**：在线按钮排前面，本地折到后面并标「或接本地服务（可选）」。
- 文案改为「**推荐用在线 API**（填 API Key 即可，无需下载任何东西）」，
  本地降为"已装的话也可以接"。
- **base URL 一律可改**（与 memo.ac 一致：24 家里 22 家可编辑）——
  国内常走中转网关，写死会直接把人挡在门外。

### ② 两处已共用同一数据源

新增 `features/settings/llm-catalog.ts`：`LLM_PRESETS` + `MODELS_BY_PROVIDER` + `useLlmConfig()`。
「AI 模型」与「按用途分别配置」**都从它取** provider 列表与 `modelsFor()`。

关键一条：`modelsFor()` **一定把用户自己配的模型名并进候选**，
否则他在「AI 模型」填了个我们清单里没有的模型，分档下拉里就找不到 —— 那正是他投诉的本体。
模型仍是**自由输入 + datalist 候选**，不是死下拉：厂商上新比我们发版快，写死会把新模型挡在外面。

有一条测试**跨两个区块比对候选集合**，那边有这边没有就直接红。

### 你要的那条"测对面"的断言（已加）

键名取自 `@openmemo/shared` 的 `LLM_SETTING_KEYS` —— 与 daemon 的 `resolveConfiguredProvider()`
同一份清单。测的不是"我发了什么"，而是"**对面会读的键有没有被写上**"。
daemon 以后新增必读键，这条会立刻红，而不是等用户去撞 `LLM_NOT_CONFIGURED`。
`llm.purposes` 也顺带对齐了（`LLM_PURPOSES_KEY === LLM_SETTING_KEYS.purposes`，有断言）。

### ④ UI 已能看出当前生效

`llm-effective` 一行直接读 **daemon 会读的那两个键**，而不是读 `llm.providers`。
照自己的清单显示就永远显示正常、永远发现不了缺键；照对面读的键显示，缺了一眼看得见。
缺任一时显示「未生效 —— 保存一个服务商即可启用（摘要/导图需要它）」。

### 验证

`tsc` 0 · `eslint apps/web` 0 · `vite build` ✓ ·
测试 **101 条 / 99 pass / 0 fail / 2 skip**（本轮新增 9 条）。

### 诚实声明 —— **验收标准我没有完整满足**

你要的是"清掉手工补的两个键 → **纯用 UI 保存** → 触发 F4"。
**我没在 demo 上做这件事**，两个原因：

1. 我**无法从命令行驱动真实浏览器 UI**，只能重放它的 HTTP 请求 ——
   那证明不了 React 那侧的接线，说"纯用 UI 验过了"就是假话；
2. 清掉那两个键会**立刻弄坏用户正在用的 F4**，而我没有能力用真 UI 把它恢复回去。
   用户此刻在用这个实例。

所以我改在隔离实例上**从破损形状复现到修复**（证据如上），**没有动 demo 一个字节**。
**请让能开浏览器的人补最后一步**：在 demo 上清 `llm.defaultProviderId` / `llm.defaultModelId`
（别碰 `llm.providers` 与 secrets）→ 打开设置页点一次保存 → 跑 F4。
我这侧能保证的是：那次保存**会**把两个键写上（有测试钉住，且隔离实例上同样的 patch 已跑通）。

---

## T-108 补充 + T-112 起步（2026-08-03）

### ① 导图保存已打开 —— 但我**删了常量**，没有改成 `true`

daemon 的 `PATCH /api/notes/:uid/mindmap`（`content.ts:329`，validate + revision）契约与
前端 `useSaveMindmapMutation` 发的 `{doc}` **完全对得上**，改 `true` 就能通。

但我按你说的"**这类常量必须与后端有没有绑定，不能靠人记得回来开**"做了：
**`MINDMAP_SAVE_SUPPORTED` 整个删掉。**
理由写在代码注释里 —— 改成 `true` 只还这一次的债，**下个功能还会重演**。
让"后端有没有"自己说话：直接发请求，端点不存在时 `client.ts` 的端点级记账 404 →
**写操作如实抛错**（写永不静默回落 mock），UI 显示错误。
既不需要有人记得回来开，也不会像最早那次把整个 `notes` 面毒化成 mock。

**与我删 `App.tsx` 的 `pending` 分支同一条原则：没有这个开关，"忘了打开"就不可能再发生。**
顺带删掉了那条"编辑尚未持久化"的提示（它现在是假的），改成**只在保存失败时**才出声。

### 全仓搜"临时关闭的开关" —— **只剩这一个，已清零**

搜了三类：`*_SUPPORTED|ENABLED|AVAILABLE` 常量、`show*={false}` 类属性、`TODO 等后端`。

| 命中                                     | 判定                                                                             |
| ---------------------------------------- | -------------------------------------------------------------------------------- |
| `MINDMAP_SAVE_SUPPORTED = false`         | ✅ **本轮删除**                                                                  |
| `showModel={false}`（录音页）            | 已由 `ui-polish` 打开，现无命中                                                  |
| `AsrModelPicker` 那条注释                | **仍属实**：per-task 引擎/模型覆盖后端确实没有入参（T-075 实测），不是关着的开关 |
| `SUPPORTED_LOCALES` / `MOCK_ENABLED_KEY` | 正常常量，非开关                                                                 |
| `RecorderPage:378`                       | mock 字幕文案，非开关                                                            |

并加了一条**回归断言**：`MINDMAP_SAVE_SUPPORTED` 必须保持 `undefined` ——
谁再加回来测试直接红。

### ② 详情页导图 tab 的只读态已明说

之前没传 `onChange` 且一个字不说，而渲染器的乐观更新**让它看起来像改成功了** ——
这是最坏的一种沉默。现加一行「此处为预览，改动不会保存」+「去导图页编辑」跳转。
（选"明说 + 跳转"而非"接上编辑"，是为了保持详情页轻量；真编辑在导图页。）

### ③ 标签与数据源已统一（承接上一轮）

- **标签统一**：两处都叫「模型」「服务商」（原来是「模型名」vs「模型」）。
- **模型控件**：两处都改成 **`<input list=…>` + `<datalist>`** —— 有下拉候选、
  同时允许自定义输入。**没做成死 `<select>`**：厂商上新模型比我们发版快，
  写死会把新模型挡在外面（这也是 `model-mgmt` 说的"该有下拉的地方给下拉"，但不能只有下拉）。
- **同一数据源**：`llm-catalog.ts` 的 `useLlmConfig()`，两处共用；
  `modelsFor()` **一定把用户自己配的模型名并进候选**。有一条测试**跨两个区块比对候选集合**。
- 服务商呈现：「AI 模型」是卡片（要展开填 Key/BaseURL），分档是下拉（只选不填）——
  这两者职责不同，我**没有强行统一成同一控件**；统一的是**数据与标签**。
  若你认为形态也必须一致，我再改，但那会让分档区变得很重。

### ④ "离线模式"：按你说的只改取向，不去消灭一个不存在的字符串

预设已扩到 11 家（在线 9 + 本地 2），**在线排前、本地折到后面并标「可选」**，
文案改为「推荐用在线 API（填 API Key 即可，无需下载任何东西）」。
等你拿到用户截图再看是否另有出处。

### T-112：**契约未到，我只做了收口，没有猜字段**

新增 `lib/api/auth-mode.ts`：`AuthMode = 'none' | 'token' | 'unknown'` 与 `authRequired()`，
作为 CSRF 内存副本 / 401 自愈 / 403 自愈 / 写前补握手**四处的唯一判断点** ——
免鉴权下它们全是空转，各判各的必然有一处先忘。

判据优先级刻意写成：**① 服务端明说（health 字段）→ ② 观察到的行为 → ③ unknown 按"要鉴权"处理**。
第 ③ 条取保守方向：多带一个头不会坏事，**少带会静默失败**（今天刚栽过）。
⚠️ `readDeclaredMode()` 目前是**宽容读**（认 `auth` / `authMode` / `authRequired` 几种写法），
**这是等契约的临时措施，不是我定的契约**。`oss-scout` 定稿后请我收紧成单一字段。
**在字段落地前我没有改变任何默认行为** —— 现在仍走完整握手，不会误判成免鉴权。
按你说的：免鉴权模式**不新增任何横幅**。

### 验证

`tsc` 0 · `eslint apps/web` 0 · `vite build` ✓ ·
测试 **104 条 / 102 pass / 0 fail / 2 skip**（本轮新增 3 条：PATCH 真的发出且路径/体正确 ·
常量必须保持删除 · 端点不存在时如实报错）。

### 诚实声明

- **没碰 demo**，本轮零请求。
- **导图保存没在真浏览器里拖一次验证** —— 证据是契约比对（路径/请求体/响应字段逐项对上）
