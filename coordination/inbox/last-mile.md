...
## [2026-08-10 14:10] T-193 DONE —— 探测端点那份"永不失效的缓存"

交付: 提交 `3944c75`（7 个文件，`git commit --only <pathspec>`，零 `-A`、零 `stash`）。

# 回报前两句（你点名要的）

> **① 两份快照我判成：都不合并，也不是"给第二份补一条失效规则"就完事 ——
> 而是按"贵不贵 / 变不变"把 `HardwareInfo` 这个 blob 拆开。**
>
> **② 装完东西之后磁盘那一格真的变了。** `[实测]` 普通 GET（不带 `?refresh=1`）：
> `35555 MB` → 写进 700 MB → **`34821 MB`**（Δ 734 MB）→ 删掉 1300 MB → **`35975 MB`**。
> 同一过程中 `detectedAt` **一次都没动** —— 也就是说没有为了刷一个 `statfs` 白跑一次 probe。

---

# ① 判断：为什么不是"合并"，也不是"各自加规则"

两份快照**同源**（都出自 `detectRuntimeHardware()`），所以第一直觉是合并。但先按你说的
"不要默认合并就是对的"往下问一层，发现**它们装的是两类寿命完全不同的事实**：

| 类 | 字段 | 取一次的代价 | 什么时候变 |
|---|---|---|---|
| **结构事实** | cpu / gpus / backends / unifiedMemory / os | **spawn**：`runProbe`(≤10 s，ADR-003 决策 3) + `detectGpus`(nvidia-smi / `system_profiler`，秒级) | 换机器、装卸后端包、换驱动 |
| **环境读数** | **`disks[].freeMB`**、`ram.availableMB` | `statfs` ×2 + `os.freemem()`，**零 spawn、微秒级** | **一直在变**（任何一次下载，甚至别的程序） |

**缓存的存在理由（"探测要 spawn"）只对第一类成立。**
缓存第二类**不省任何东西** —— 而代价是界面上那个数是假的。
所以正确的切法不在"几份快照"这个轴上，在"**这一格该不该被缓存**"这个轴上：

- **环境读数：根本不进缓存**，每次请求现算。
  ⇒ 你的判据 2（下了 112 MB 之后那一格必须变）**由构造成立**，
  与任何失效规则是否触发无关 —— 哪怕断路器一辈子不跳。
- **结构事实：保留缓存，但补上真实的失效规则**（下面 ②）。

**为什么两份不整个并成一份**（如实说，这是取舍不是原则）：两份的**输入本来就不同** ——
`hardware.ts` 拿到完整 `AppPaths`（真 `dataDir`），`state.ts` 只有 `modelsRoot` 得反推；
`state.ts` 还要把用户显式选的 `selectedBackend` 盖回去。合并要先统一输入，那是另一件事，
而且 `state.ts` 此刻在别人手里（见 §纪律）。**但失效规则可以先统一，而那才是这次的病。**

## ② 第二份的失效规则：与第一份**逐字同一条**，不是新造一条

`hardware.ts` 那份此前唯一的失效方式是 `?refresh=1`，而 `?refresh=1` 在整个网页里
**只有一个调用点**（`useHardwareRefresh`，断路器 open→closed）。
现在它跟 `state.ts` 用**同一个** `canonicalPackFingerprint`（按内容不按 id，T-191 那一版）。

**复用而不是抄一份**：抄一份就是两条规则，而"同一件事在两处各算一次、只修了一处"
正是这次要修的病本身。代价是 `hardware.ts` 与 `state.ts` 之间多了一个**调用期**的循环引用
（两边都只在函数体里用对方，模块初始化期零依赖；已在真 daemon 上跑通）。
**真正的解法是把指纹提到一个共同的下层模块** —— 那要动 `state.ts`，留给它的主人。

`[实测]` 装一个包 → `detectedAt` 变；**同 id 换不同字节** → 变（T-191 那个洞）；
卸载 → 变；**什么都没变 → 不变**（不能每请求 spawn 一次 probe）。

## ③ 顺带那条（同一个根，一并治了）

卡片副标题渲染的一直是 `pack.engineVersion` —— **目录的**。于是
`已安装 · ffmpeg n8.1.2-… · 112 MB`，而机器上是 n7.1.5。
**「已安装」+ 一个它并不拥有的版本号，连起来读就是一句假话。**

端点补发 `installedEngineVersion` / `installedSizeBytes`（体积按**安装记录里各文件求和**算，
不复用目录的 `totalSizeBytes` —— 换版本时体积往往也变了）。三档：

| | 显示 |
|---|---|
| 没装 | 目录的版本与体积（那正是你点下去会拿到的） |
| 装了且最新 | 机器上那一份（与目录同值，读起来无差别） |
| **装了但有更新** | **两个都说**：`ffmpeg n7.1.5 · 12 MB · 目录里是 n8.1.2（112 MB）` |

没装时是 `null`，**不是目录值也不是缺失** —— "我没有"和"我不知道"都不该被渲染成
"和目录一样"，那正是这句假话的来源。老 daemon 不发这两格时退回老行为，
**不替它说"你装的就是最新版"**（与 `updateAvailable` 同一条判据）。

---

# ④ 顺带判：`?refresh=1` 挂在断路器状态转换上，本身是不是缺陷？

**不是。缺陷在于它曾是"唯一"的那一个。**

- 断路器恢复那一刻重探是**正当且必要**的：断路器状态是 runtime 包的**进程内**状态，
  不落进任何 manifest，**指纹看不见它**（`state.ts` 为此专门留了 `invalidateHardware()`）。
  它必须有一个显式触发点，而"恢复成功那一刻"恰好是代价最低的时刻
  （shader 缓存刚被捂热，T-172 实测 17606 ms → 163 ms）。
- 真正的缺陷是**它成了唯一入口**：一个刷新机制的触发条件是另一件不相关的事，
  于是所有与断路器无关的变化（装包、下模型、磁盘变动）**无人负责**。
- T-193 之后它回到本来的角色：环境读数不需要触发点（实时），装卸换包由指纹负责，
  `?refresh=1` **只服务"指纹看不见的那一类"**，断路器恢复正是其中之一。

> **判据：一个显式刷新入口本身没问题，问题是它成了唯一入口。**
> 现在它是兜底，不是主路径。

🟡 **一条我没做的**：那张卡上仍然**没有用户可见的"重新检测硬件"按钮**。
改完之后已经基本不需要它了，加一个"点了看不出区别"的按钮是另一种假 UI。**请你裁要不要加。**
...
| 项 | 处置 |
|---|---|
| 更新后旧文件不回收（~279 MB 死重） | ⛔ **没碰**，按你说的归另一路 |
| **后端包路径不显示百分比** | ✅ **只判、未改**（见下） |

## 判：不是"有意不显示"，也不是数据缺失，是**卡片上根本没有进度 UI**

`[读码]` 三条事实：

1. `backends.ts:198` 建 job 时 `totalBytes: pack.totalSizeBytes` —— **非 0，数据是齐的**；
2. `JobList.tsx:143-153` 是 `完成/总量` 文本 **+ `ProgressMeter`** —— **百分比在那儿**，
   而且**没有按 job kind 分支**（`jobToastModel` 里的 `kind` 只用于区分下载类/流水线类）；
3. `RuntimePage.tsx:121` 给卡片传的只有 `installing={install.isPending && ...}` ——
   一个布尔，而且 202 一回来就变 false。**卡片从头到尾拿不到字节数。**

⇒ 所以"后端路径只有字节/速度/ETA"应该是在**共享任务面**上看到的，而卡片那侧是**零进度**。
要补的话落点是 `RuntimePage` / `BackendPackCard`，**但 `DownloadRow.tsx` / `JobList.tsx` /
`JobToaster.tsx` 此刻都在别人手里**（`git status` 都是 M）。**请定归属，我不抢。**

---

# 腿（判据是"用户看到的数是真的"，不是"缓存被打掉了"）

`apps/daemon/src/http/rest/hardwareLiveReadings.test.ts`，**10 条，走真 daemon**。
**一条"断言 detectDisks 被调用了"的用例都没有** —— 那种用例在缓存回来的那天照样是绿的。

核心那条：真写进 256 MB → **普通 GET** 回的可用空间必须降 ≥128 MB。
配一条**反向**的：同一次里 `detectedAt` **不许**变。
少了后者，"把缓存整个关掉"也能让第一条变绿 —— 而那会让每个请求 spawn 一次 probe，
**把"缓存永不失效"换成"缓存永远无效"，同一枚硬币的另一面**。

## 反向验证 4 组，**全部跑在 `/tmp` 的隔离副本上**（PROTOCOL §10）

上一轮我在共享树里做反向验证、事后才申报。这次改了做法：把 `apps/daemon/dist` 拷到
`/tmp/lm-rev-t193/run/`，`node_modules` 用符号链接指回真的，**只在副本里改坏产物**。
共享树的 `dist` **全程没有坏过一秒**。每组跑之前都 `grep` 确认坏行在即将运行的产物里。

| # | 撤掉什么 | 真实输出 |
|---|---|---|
| **R1** | 拿掉实时读数（回到 T-193 之前） | `✖ 写进了 256 MB，而端点回的可用空间只降了 0 MB` / `删掉 256 MB 之后没涨回去：35969 MB → 35969 MB` |
| **R2** | 把缓存整个关掉（每请求重探） | `✖ detectedAt 跟着变了 —— 说明为了刷新一个 statfs 就重跑了整套探测` + `✖ 两次连续请求探测了两次` |
| **R3** | 拿掉指纹失效（只剩 `?refresh=1`） | 装 / 换字节 / 卸 **三条全红**：`✖ 装完一个后端包，硬件快照没有重算` |
| **R4** | `installedEngineVersion` 退回目录值 | `+ 'n8.1.2-34-g9b6c8969e0'  - 'n7.1.5-fake-old'` —— **用户真机症状的逐字复现** |

⚠️ R4 第一次跑出来的红是**假红**：隔离副本里 `resolveManifestDir()` 上溯 5 层落到 `/tmp`，
目录是空的，于是"前提自检"报 `目录里没有 media-tools-linux-x64`。
**那条前提自检替我挡住了一次无意义的绿/红** —— 补 `OPENMEMO_MANIFEST_DIR` 后才是真红。

---

# 门禁

`pnpm lint` 0 · `npx tsc -b` 0 · `pnpm -r test`：
`shared 52` · `db 53` · `llm 18` · `downloader 53` · `mindmap 48` · `runtime 130` ·
`pipeline 243` · `web 172+10+340+18` · **`daemon 642 pass / 1 fail`**

## 🔴 那 1 条红**不是我的**（逐条定位）

`apps/daemon/src/storage/move.test.ts:425` 的
`★★ 调用方：扫不全时不许报成干净 —— warningZh 必须说出来`。

`[实测]` `apps/daemon/src/storage/move.ts` 工作区里 **+117 行**，而 `StaleLinkScan`
这个类型**在 HEAD 上根本不存在**（`git show HEAD:…/move.ts | grep -c StaleLinkScan` = 0）
—— 有人正在把 `findStaleLinks()` 的返回值从数组改成对象，测试还没跟上。
**这正是 PROTOCOL §10 那条推论的字面形态**：「必填字段类的改动，在补齐所有构造点之前，
红是必然的、不是意外」。我这次是接收方。

⚠️ 顺带记一次：14:00 左右我连跑两次 `tsc -b`，第一次报
`move.test.ts: Cannot find name 'mkdirSync'`，20 秒后再跑就没了 ——
**对方正在保存那个文件的中途被我读到了**。我没有去"修"它。
...
| 条 | 结果 |
|---|---|
| `:10000` | ✅ **零请求**，pid `491899` 仍在，未重启/未 kill/未占用 |
| `/root/data-memo` · 模型 · 活动模型 | ✅ 一个字节没碰（mtime 仍是 `2026-08-10 12:57:38`） |
| 机器级指针 | ✅ sha256 `7f930979…0da233f3`，未变 |
| `pkill -f`（含 `-0`） | ✅ 未用。我起的验证 daemon 在 **19500**，只 `kill` 了自己记下的 pid，已确认端口空出 |
| 反向验证 | ✅ **全程在 `/tmp` 隔离副本**（PROTOCOL §10），共享树 `dist` 未坏过；`grep REVERSAL` 全仓 0 |
| `git` | ✅ `git commit --only <7 个 pathspec>`；新文件先 `git add`；**零 `-A`、零 `stash`、零 `--amend`** |
| 别人的在途改动 | ✅ `move.ts` / `move.test.ts` / `manifestLoadFailure.test.ts` / `silentFailures.test.tsx` / `backendStatusUnion.test.ts` **一个都没 add**（提交后 `git status` 逐条核对） |
| release / 分支保护 / 可见性 | ✅ 未碰 |

**未 push** —— 你要求 push 用具体 hash 并 `merge-base --is-ancestor` 复核，
而此刻还有 3 路在跑、`master` 随时在动。**要不要我推、推哪个 hash，你说一声。**

---

# 需要你裁

1. **`state.ts` 那份快照有同样的病，而且后果更硬**：`computeFit()` 吃的是它的
   `hardware.disks`，也就是说**「空间不足 / blocked_disk」这个判定也建立在一个旧的可用空间上**。
   `[读码]` `state.ts:1023` 把 `this.hardware` 喂给 `computeFit`。
   修法与本轮**一模一样**（把那两格改成现算）。**我没动**：那个文件此刻在别人手里，
   上一轮我已经因为"暂存到提交之间的窗口"被别人的提交扫走过一次。**请派给它的主人。**
2. **同一个数第三处**：`/api/models/storage` 用的是 `volumeBytes()`（**实时**）。
   于是全仓"可用空间"有三个答案：实时 / 结构缓存 / fit 缓存。本轮修掉两个，第 3 条见上。
3. **要不要给硬件卡加一个用户可见的「重新检测」按钮**（④ 末尾）。我倾向不加。
4. **后端包卡片要不要有进度条**（归属见上）。
5. **push 与否**。
