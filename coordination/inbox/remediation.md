...
## [2026-08-11 02:15] T-200 S-4 追问 DONE

**`c71f92e`（假红灯修复）· `ca5cc46`（新问题面的腿）。CI：`31411608496` success · `31412574662` success。**

### 锚点那半：没碰，但独立验过

`scripts/mutation-check.mjs` 是 CI 治理文件，不在我授权范围内 —— 我**一行没改**（更早一次尝试改共享治理文件时被权限系统拦下，拦得对）。`aaf74bf147d7e5170` 的 `b1e1543` 重指了锚点，我独立复验：`--only E3-write-no-mock-shortcut` **1 条被测出 0 条存活**，`--anchors-only` 18 条全命中，完整 18 条全红 0 存活。**"锚点命中"与"规格有牙齿"两档都过了。**

### 正面回答：窗口内的写会不会被误放进 mock

**不会，而且有两道互相独立的防线：**
1. `const missRec = isWrite ? undefined : …` —— 写永远绑不到记录，早退对写**结构上不可达**；
2. `endpointKey()` **把 method 拼进键** —— `GET /x` 与 `PATCH /x` 是两把不同的键，就算第 1 道被拆，读的钉住记录也落不到写头上。

另：`client.ts` **没有任何按 path 合流的在途请求表**（grep 过），所以"探路在飞"与并发的写之间不共享 `missingEndpoints` 以外的状态。

此前**只有第 1 道被变异覆盖，第 2 道是裸的**。补了三条腿并逐条验牙齿：拆第 1 道 → 既有 E3 用例 + 「窗口内再写」红；第 1、2 道一起拆 → 「读被钉住时同路径的写」也红（专钉第 2 道）。

⚠️ 第三条（并发探路）**如实标注为钉不住这两道防线**：拆任意一道乃至两道它都不红 —— 因为「探路在飞」与「早退生效」是**互斥状态**（窗口没过不探路，窗口过了谁都进不了早退），这个时序**结构上就没有洞**，不是"我们守住了"。留着当**前提哨兵**（哪天有人加了按 path 的在途合流它会红），并在注释里写明"别把它算进覆盖率里"。

### 🔴 顺带查出并修掉我自己在 S-4 种下的一个假红灯

追这条时发现：S-4 给 `missingEndpoints` 记了 surface 之后，`hasMissingOnSurface()` 数的是该面**所有**缺失记录，而 `isNotImplemented` 那一支**对写也记账**（`PATCH /notes/:uid/mindmap` 正是当年那次事故的正主）。于是**一条缺失的写路由 ⇒ 该面任何一次成功的读都把它标成 `mock`** ⇒ 界面挂出「这些数据是假的」，而实际一个读端点都没回落（写是如实抛错的）。

这正是文件开头那段事故注释里「按 surface 记会毒化整个面」**从标注这道门又走回来一次**：路由层已按端点记，标注层却退回按面记。**假红灯与假绿灯同罪。**

修法：记录带 `isRead`，`hasMissingOnSurface()` 只数读。**先写腿、先看它真的红，再修**；反向验证 2 组（把写重新算进来 → 假红灯那条红；标注做成永不挂 → 2 条红含反向鉴别）。
