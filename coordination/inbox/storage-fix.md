...
## [2026-08-10 16:20] T-167 DONE —— listModels 的失败不再伪装成「这家没有模型」

### 回报你要的两句

**第一句：腿写完是红的。** 12 条,实现之前**编译就是红的**(`Cannot find module './list-models.js'`);
把 `catch → return []` / `!res.ok → return []` / 形状不对 `?? []` 三处恢复成原缺陷行为
(签名保持不变,只退行为)后,**12 条里红 8 条**。最能说明问题的是这一句:

```
✖ ★★★ 五种处境的「结论 + 下一步」必须两两可辨
  AssertionError: 「401」与「真的空」给出了同一个答案（ok:0）—— 这正是 return [] 的病
```

**第二句：401 / 断网 / 真的空列表,用户各看到什么。**

| 处境 | code | 用户看到 | 下一步(按钮) | 可重试 |
|---|---|---|---|---|
| 200 + 空列表 | — (`ok:true, models:[]`) | 这家**确实**没有模型 | 无 | — |
| 401 / 403 | `LLM_AUTH_FAILED` | **API Key 无效或已过期** | **给** `openSettings` → `/models?tab=llm` | 否 |
| 连不上 / DNS 失败 | `LLM_CONNECTION_FAILED` | 连不上 LLM 服务(`<baseUrl>`) | **给** `checkLocalBackend` → `/models?tab=llm` | 是 |
| 超时 | `LLM_TIMEOUT`(**新接线**) | 连接超时,可以重试 | 不给 | 是 |
| 429 | `LLM_RATE_LIMITED` | 频率超限,会自动重试 | 不给(`wait` 已认领为"刻意不给") | 是 |
| 5xx | `LLM_SERVER_ERROR` | 服务暂时不可用 | 不给 | 是 |
| 形状不对 / 非 JSON | `LLM_BAD_RESPONSE` | 这个地址多半不是模型列表接口 | 不给 | 否 |

### ⚠️ 一处必须先说的更正:这两条**当前的用户可见后果不成立**

你转述的危害是「用户 Key 填错 → 产品说"这家没有模型" → 他去换服务商」。
**我复核后认为这在今天的产品里不会发生,理由是这两个函数根本不在用户路径上。**

- 全仓 `listModels` 的调用方只有两处:`packages/mindmap/src/mindmap.test.ts`(mock)
  与 `packages/mindmap/scripts/demo-f4.mjs`(演示脚本)。**产品代码零调用。**
- 用户点「刷新模型列表」走的是 `POST /api/llm/models` → `apps/daemon/src/llm/enumerate.ts`,
  那是**另一份实现**,而且**已经是对的**:HTTP 非 2xx、形状不对、甚至"枚举到 0 个"
  都各自返回带 `kind`+`detail` 的失败,并且有两条腿钉着
  (`llmRoutes.test.ts:223` 「枚举到 0 个算失败 —— 空下拉会被读成"我的账号没有模型"」、
  `:258` 「上游 4xx/5xx 原样带回状态,不许当成"没有模型"」)。

**所以这两条的真实定性是:一个仍然装着子弹的陷阱,不是正在流血的伤口。**
它是 `@openmemo/llm` 的**导出公共接口**,下一个把"刷新模型"接到它上面的人会原样踩中。
修它是对的,但我不把它报成"修好了一个用户正在撞的 bug" —— 那会污染优先级判断。

### 2. 修法(与本族同一条判词)

`LlmProvider.listModels()` 从 `Promise<string[]>` 改成:

```ts
export type ListModelsResult =
  | { readonly ok: true; readonly models: string[] }     // 空数组 = **合法的零**
  | { readonly ok: false; readonly error: LlmError };
```

*旧签名在结构上没有能力区分"真的零"和"没问到",于是调用方也不可能小心。*
签名一改,**编译器当场把每个实现和每个调用点都指了出来** —— 这正是它该有的效果。

判据仍是 T-166 那条:**不是看 errno,是看「这个位置在当前语境下本来就该不该有东西」**。
对面**好好地**回了空列表 ⇒ 真的零;"好好地回答"这个前提一旦不成立(HTTP 错/网络错/
形状不对/不是 JSON)⇒ 不是零,是**没问到**。

新增 `packages/llm/src/list-models.ts`(一份实现,三个 provider 共用),另在 `errors.ts`
接上 `LLM_TIMEOUT`:`AbortSignal.timeout()` 抛的是 `name==='TimeoutError'` 而非 `AbortError`,
此前没有这一支,超时会掉进 `LLM_CONNECTION_FAILED` 和"连不上"说同一句话 ——
可这两件事的下一步不同(改地址 vs 重试)。顺带给列模型加了 **10s 超时**(此前没有,会一直挂)。

### 3. ⚠️ 申报:我动了第三个文件 `providers/gemini.ts`(你只点名了两个)

**不是我扩范围,是编译强制的**:它实现同一个 `LlmProvider` 接口,返回类型一改它就编译不过。
而且留着它用老形状会**立刻**制造"同一个方法两种契约",正是这一族的病根。
它的 `catch { return []; }` 旁边原本还写着「列模型只是便利功能,失败不该冒泡成错误」——
那句话正是这个缺陷的自我辩护,已删。

**这不是你说的"第 11 处"**(那指清单里的新条目)。`listByName()` 与其余几条**我没碰**。

### 4. 按钮给不给:逐条判过(你的第 3 条)

`remediation/routes.ts` 记着 **26 个调用点里 23 个刻意不给**,「给一个点了没用的按钮,
比不给按钮更糟」。我的判断:

- **401/403 → 给。** Key 是用户**自己能修好**的,落点明确 —— `openSettings` 已经路由到
  **`/models?tab=llm`**(`routes.ts:90`),就是你说的那个位置,**不是 `/settings`**。
- **连不上 → 给。** `checkLocalBackend` 同样落在 `/models?tab=llm`(`routes.ts:91`),baseUrl 在那儿配。
- **超时 / 5xx / 429 → 不给。** 下一步是"再试一次/等一会",那是**就地动作不是跳转**,
  `retryable: true` 已经表达完;跳任何页都是原地转圈。
- **形状不对 → 不给。** 沿用既有 `LLM_BAD_RESPONSE` 的处理(它一直没有按钮)。
  话已说清"这不像 /models 端点",但**该改成什么地址我们并不知道** ——
  给一个跳转按钮等于假装我们知道。

⚠️ **两个动作名都是既有的、已在 `REMEDIATION_ROUTES` 里认领过**,所以没有新增
"发得出却没人接"的引导。**并且我确认了那道守卫真的会喊**:临时塞一个
`{ action: 'inventedActionForGuardProbe' }` 进去 →

```
✖ ★ 每个 action 要么有路由，要么在 UNROUTED_ACTIONS 里写明为什么没有
    inventedActionForGuardProbe（/root/memo/packages/llm/src/list-models.ts:126）
```

它连行号都点出来了。探针已按新纪律**用备份文件撤销**(未用 `git checkout`)。

### 5. 腿(12 条)与两条防假绿的设计

- **不只断言"返回了错误"**(你的第 2 条):每条都断到 `code` + 下一步 + `retryable`。
  只断 `ok===false` 会被一句笼统的「加载失败」满足 —— 那只是把错的答案换成没用的答案。
- **`★★★ 五种处境两两可辨`**:把五种处境各跑一遍,取「code|按钮|可重试」当指纹,
  要求两两不同。**逐条断言 code 的腿抓不到"合并"**,这条能 —— 它红的时候会直接说出
  是哪两种被压成了同一个答案。
- **`★ 前提成立`锚点**:断言注入的 fetch **真的被调用了、且打在正确 URL 上**。
  没有它,上面所有用例都可能是在"根本没发请求"的情况下通过的。
  (T-166 那轮我吃过一次假红,这轮先把假绿堵掉。)
...
eslint .（全仓库）= 0
pnpm -r test      = 0
  daemon 653 · web 344+172+18+10 · pipeline 243 · runtime 133 · downloader 82
  · db 53 · shared 52 · mindmap 48 · **llm 30（新增 12）**
守卫「daemon 会发的 action 前端必须逐个认领」172/172 绿，且已实证它扫得到我的新文件
```

`apps/web/dist` mtime `2026-08-10 12:56:16` —— 不是我的命令产生的(全程没跑 vite build)。

### 7. 自查

- **最要紧的一条:我没有顺着你的前提走。** 危害描述("用户会去换服务商")在今天的产品里
  不成立,因为这两个函数没有产品调用方,而真实路径 `enumerate.ts` 早就修对了。
  修仍然值得做(公共接口 + 演示脚本 + 下一个人必踩),但**定性要准**,否则会污染优先级。
- **`gemini.ts` 是编译强制的连带改动**,已在 §3 单独申报,不是我自作主张扩范围。
- **与 `enumerate.ts` 的判定差异是刻意保留的**:那边把"0 个"也判失败(产品决策),
  这边把"200+空列表"判成合法的零(库原语如实转述)。两者语境不同、都对,
  已在两处注释里互相点名**不许"统一"** —— 与 T-166 的 ENOENT 处理同一手法。
- **给列模型加了 10s 超时属于行为变更**(此前无超时会一直挂)。我认为这是修复的一部分
