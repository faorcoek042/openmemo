...
## ⚠️ 更正（同一轮，必须自己说出来）：我的回执提交 `b5b39f3` 卷进了别人的三个文件

`b5b39f3` 的 `--name-only` 实际是 **4 个**文件，不是 1 个：

```
coordination/inbox/sweep-fix.md      ← 我的
docs/DEPLOYMENT.md                   ← 不是我的
docs/SECURITY.md                     ← 不是我的
docs/design/D-20-bundled-deps.md     ← 不是我的
```

**成因**（重要，是这个共享工作树的结构性坑）：
我先跑 `git add coordination/inbox/sweep-fix.md`，并且**当场用
`git diff --cached --name-only` 核对过 —— 那一刻确实只有 1 个文件**。
但 `git commit` 是**下一条命令**，两条之间另一路 agent 做了他自己的 `git add`，
把三个 docs 放进了**同一个索引**。而**不带 pathspec 的 `git commit` 提交的是整个索引**。

> **判据：在共享工作树里，"我 add 完核对过了"不成立 —— 索引是全仓共享的可变状态，
> 两条命令之间它会被别人改。核对的是过去，提交的是现在。**
> **修法不是"核对得再仔细一点"，是 `git commit -- <明确路径>`**（带 pathspec 时
> git 只提交这些路径，绕开索引里别人的东西）。这条与 §12「新文件要先 `git add`，
> 因为 pathspec 对未跟踪文件是静默 no-op」是同一枚硬币的两面：
> **新文件必须先 add，然后仍然要用 pathspec 提交。**

**我没有回滚它。** 那三个文件的内容是完整的成品（2026-08-10 的许可证署名订正，
`lib.min.js` 的复合许可证清单等），回滚等于毁掉别人已完成的工作；
内容已经在 master 上、没有丢失也没有损坏。**只有署名错了** ——
它们挂在我的 commit message 下面。

**对那一路的影响**：他再 `git commit` 时会发现这三个文件"没有东西可提交"。
**请 Manager 转告，并按需决定要不要补一条说明提交。**

（本条更正本身用 `git commit -- coordination/inbox/sweep-fix.md` 提交，
不再重复同一个错误。）

---

## [2026-08-10 续] 徽标落地 · 13+5 清完 · 关键词判据全仓清单（29 条）

**提交** `4cb083b`（已 push，`merge-base` 复核通过）。
`tsc -b` 0 · `eslint` 0 · `prettier --check` 通过 · apps/web **502 条 0 failed**（158+10+325+9）。

# 回报三问

1. **徽标落地没有** —— **落了**。侧栏「任务」现在带「进行中 N」。
2. **13+5 清到第几** —— **全清完，18/18**。
3. **关键词判据全仓有几处** —— **29 条**（A+B 6 / A 15 / B 8），另 **~520 条判为正当（C），不报**。
   **按你说的先出清单，一条都没改。**
...
口径取 `@openmemo/shared` 的 `TERMINAL_JOB_STATES`，**不另写一份** ——
任务中心 / Toast / 徽标三处对"结束了没有"必须同一个定义，
分叉的表现是「徽标写 1、点进去什么都没有」，而两个数字各自都算得对。

- `blocked` **算**：它永远不会自己结束，正是最该把用户叫过去的那一种；
- `failed` **不算**：终态且永远留在列表里，算了**徽标永不归零 = 徽标疲劳 = 等于没有徽标**
  （⑤B「假红会训练人忽略告警」同一条）。

**计数进 `aria-label`**，不只做一个视觉小圆点 —— 这个徽标存在的全部理由就是"告诉用户还有活在跑"，
对读不到它的人来说等于没做。

**刻意不重放已发生的 `job.created`**（你已认可）：Toast 是瞬时通知，
重放会让每次刷新被一堆旧通知糊一脸，比现在更糟。

⚠️ **与 `af01cc1c…` 的关系**：我这条解决「不知道还有任务在跑」，
它那条（任务中心点不进笔记）解决「点不进去看结果」——**两条合起来任务中心才真的是那个持久层**。
我已把改动范围同步给它：我**没碰** `MergedJob` / `mergeOne` / `JobRow` 标题行，零重叠。

---

# §2 13 + 5 全部清完

**判据不是"代码里有 catch"，是"用户在屏幕上真的被告知了"** —— 这两件事不是一回事，
B11 那条 404 早就在说话而判据看不见，正是活证据。

## 13 处完全不出声

| 位置 | 修之前用户看到 |
|---|---|
| `JobList` 暂停/继续/重试/取消（4） | 什么都没有。⚠️ 上面那个 `<p>` 渲染的是 **job 数据里的 error**，不是这四次 mutation 的 —— 长得像"已经处理了"，这是它漏这么久的原因。暂停/继续服务端回 **501 + `cancel_job` remediation**，**服务端算好的建议在产品里到不了屏幕**；`ErrorBlock` 默认渲染 remediation，接上即活 |
| `FolderTree` 新建文件夹 | 输入框合上、树上没有新文件夹 = "我是不是没按到回车"。⚠️ **同一个文件下面那句注释写着「我刚清完 14 处，不新增第 15 处」——第 15 处就在这个文件里、在那句话上面**：改名和删除都接了，`create` 漏了 |
| `AsrModelPicker` 切换模型 | `<select>` 是受控的 ⇒ **悄悄弹回旧值** = "这个下拉坏了" |
| `PurposeBindingsSection` 绑定 + 重置（2） | 值悄悄弹回继承态。⚠️ 本组件持有**自己那份** patch mutation，`LlmSettingsSection` 那个 ErrorBlock 是**另一个实例**，救不了这里 |
| `TagEditor` 加/删标签（2） | 加标签是两步，第二步失败时第一步已生效 —— **半成功状态最该说话** |
| `TranscriptList` 段落编辑/还原（2） | 只回滚缓存，文字"跳回原样"。**乐观回滚是状态正确性，不是告知** —— 这两件事一直被当成一件 |
| `NotesListPage` 星标 | 只看到"星星弹回去" |

`TranscriptList` 的错误块用 **sticky 顶在滚动容器上**，不进虚拟列表：
塞进 `getVirtualItems()` 会被虚拟化的绝对定位和高度计算算错，**而且滚出视野就消失** ——
一条会自己跑掉的错误提示等于没有。

## 5 处「渲染点在，但够不着」

**比完全没有渲染点更难发现**：读代码看得到 `isError`，只有走到那个状态才知道它不在同一棵子树上。

| 位置 | 够不着的条件 |
|---|---|
| `ModelDetailPage` 校验 | 渲染点在 `bench ? … : …` 的 **else 分支**，按钮在无条件 section ⇒ **已跑过基准的模型，校验失败零表达**。已搬到按钮同侧；顺带 `void mutateAsync` → `mutate` |
| `ModelsPage` 取消/重试/清理（3） | 错误块在 `tab==='asr' ? … : 'hidden'` 的面板**内**，三个按钮在两个 tab 下**都可见** ⇒ **`?tab=llm` 下点它们，错误块被 `hidden` 吞掉**。已搬出面板 |
| `NoteEditor` 正文保存 | 失败后 `dirty` 保持 true ⇒ 徽标停在「未保存」，而**正常打字的 800ms 防抖窗口里也是这个样子** —— "存不进去"和"我刚敲了个字"完全同形，用户会一直打到关页面才发现正文没了。现在失败是**自己一档**（critical +「保存失败」）+ 原因可读 |

> **判据：错误的渲染点必须和触发它的控件处在同一个条件/可见性作用域里。**

## ⚠️ 覆盖边界（不写成"全部已反向验证"）

18 处里**只有 `NoteActionsMenu` 那 3 处 + 徽标有逐处反向验证**（共 8 个变异全红）。
其余 **15 处是同一形状的静态修复**（`tsc` + 共享 `ErrorBlock` 契约用例），**没有逐处变异证明**。
要补的话是 15 个组件测试，建议单独排一轮；commit message 里也这么写了。

---

# §3 关键词判据全仓清单（**先出清单，一条没改**）
...
## A+B（6）—— 全在浏览器腿里，与 B11 同族

| 位置 | 判据 | 失败模式 |
|---|---|---|
| `e2e-browser-audit.mjs:1243/1286/1304` B6「安装失败要说话」 | **与 B11 逐字同一张词表** + 整页 `replace` | 安装失败文案写成「这个包这台机器上装不了／换 CPU 基础包试试」就一个词不含 ⇒ 假红；整页退化 ⇒ 假绿 |
| `:1323` B6 的**证伪腿** | 同上 | ⚠️ 它一旦失真，**不是错一条断言，是把 B6 整条的"已验证"资格伪造掉** |
| `:776-778` B5「测速失败要说话」 | 兄弟词表 + `slice(0,4000)` 截断 | **比 B6 更脆**：截断边界一移动 `before` 就不再是子串 |
| `:863-872` B6 阶段序列/倒退 | 整页 `includes` | 页面上另一条排队任务写着「排队中」就被记成"本任务阶段倒退" |
| `e2e-allcomponents.mjs:869` | `/设置\s*→\s*代理/` 于**整个 job 对象序列化** | 钉一句会被改写的自然语言（连箭头样式都钉死）；扫的是整个 JSON |
| `simulate-user-launch.mjs:152` | `/浏览器/` 于**整个启动器 stdout** | 用一个词代表"可操作性"；横幅之外任何地方出现即算过 |

⚠️ **`FAIL_WORDS` 现在还有 3 处在用（1295/1304/1323），加 776 行的兄弟词表 ——
B11 修好了，同一形状的另外 4 处一个没动。**

## A（15）—— 词表判语义，**其中几条是假绿方向，比 B11 那种假红更贵**

最要紧的三条：

| 位置 | 判据 | 为什么贵 |
|---|---|---|
| `e2e-runtime-audit.mjs:1429` | `/driver missing or too old\|驱动缺失\|驱动.*过旧/` 判「理由有没有撒谎」 | 产品换个说法（「显卡驱动版本太低」）就照样绿 —— **假绿** |
| `e2e-runtime-audit.mjs:2085` / `datadir-migrate-audit.mjs:307` | `.includes('已移动')` 判**数据搬迁的诚实性** | 措辞一改 `claimsMoved` 恒 false ⇒ `honest` 恒 true ⇒ **不可逆操作的诚实性守卫静默失效** |
| `apps/daemon/src/http/media.test.ts:247` | `/不存在\|已删除\|丢失/` 判「不许把 403 说成文件不存在」 | **否定断言** ⇒ 改口成「找不到这个文件」谎话原样通过 |

其余 12 条：`e2e-runtime-audit` `:1359`/`:1211`/`:1255`/`:1120`/`:2199`/`:2201`、
`e2e-record-assertions:518`、`e2e-browser-audit:897`、
`applicability.test.ts:191`（⚠️ **它自己的注释就写着"改成钉性质"，落地却仍是三个词的或**）、
`tool-refresh-message.test.ts:18`、`chineseEngineResolve.test.ts:177`、
`selfTestWrongCard.test.ts:145/195`、`daemon.test.ts:146`、`ready-banner.test.ts:49/74`、
`llmRoutes.test.ts:160`。

## B（8）—— **这一条我建议最先动**

| 位置 | 为什么 |
|---|---|
| ⚠️ **`scripts/check-contract-fields-shown.mjs:100-101`** | 全仓**唯一**一条"证明界面真的读了某个契约字段"的守卫，实现是**整文件、不剥注释的标识符搜索**。一条 `// TODO: 显示 engines` 注释、一个纯类型转发、一个 `'engines'` 字符串键都让它恒绿。**它防的那类损失（"一个已送到前端的事实在最后一米被丢掉"）恰好可以由一句注释伪装成已修复。** 它的文件头已经识别并挡掉了 A，**但没挡 B** |
| `check-orphan-exports.mjs:421/430/441`（**我写的**） | 判据是跨文件词边界命中计数：别处一个同名局部变量/参数/属性就把真孤儿算成"有人用"。方向是**假绿**（少报），所以不显眼。已有剥注释/失步检测/阴阳性对照，但根子仍是文本计数 |
| `proxyCoverage.test.ts:100/145/149` | 同文件 111-140 那条主断言做得很对（切到调用点、逐调用点豁免），145/149 却退回**文件级**匹配：文件里任何一处 `proxy:`（含注释）都让它绿，而**真正出网的调用点可能一个都没传** |
| `lint-workflows.mjs:217/220-227` | `JSON.stringify(整个 workflow)` 后子串匹配；**否定断言** ⇒ 一句「不要再用 X 了」的注释会让门禁红 |
| `backendNotProbed.test.ts:223/231` | 扫**整个响应体序列化**而不是 `error.messageZh`；同族的 `selfTestWrongCard.test.ts` 已经会取单字段 —— **结构上做得到，这里没做** |
| `e2e-browser-audit.mjs:714/716` | `KNOWN_DEAD` 按钮文案**子串**豁免（当前清单为空，**惰性**）；结构做法是钉 testid |
| `e2e-notes-audit.mjs:448` | 轻，只用于诊断输出，不改红绿 |

## C（~520）—— 正当，**不报**

按你要的口径区分了「钉住已知文本」与「用关键词猜语义」：

⚠️ **你点名的那 31 条锚点（`mutation-check.mjs` 18 + `e2e-runtime-audit.mjs` 13）判为 C，不是 A。**
理由三条：① 锚文本是**编译器输出的确定语句原文**（`'  state: assetStateOf(a.state),\n'` 这种），
不是自然语言，改代码必然一起改；② 两处都强制**恰好命中 1 次**，0 次或多次一律 `ANCHOR` 失效
**当场红**；③ 失配方向是**红**（已验过），不是静默假绿。
**它们钉的就是那段代码本身 —— 文本即结构。**

其余 C：`selftest-*.mjs` 断言自己的 stdout 固定格式（~127）、
`components.test.tsx` **容器内**的固定文案（~205）、
契约码/正则格式断言如 `/FORBIDDEN_ORIGIN/`、`/^sha256-[a-f0-9]{64}$/`（~151）、
`lint-workflows` 的命令行契约（~17）。

**一处反向的模范**：`packages/pipeline/src/subprocess/__tests__/sourceIsGreppable.test.ts` ——
它守的正是"`grep -r` 静默跳过含裸控制字节的文件"，而判据是**逐字节码点检查**，不是文本匹配。

## 建议的动手顺序（等你定）

1. **`check-contract-fields-shown.mjs`**（B）—— 唯一一条该类守卫，且能被注释骗过；
2. **A+B 那 6 条**（浏览器腿）—— 与 B11 同族，改法已经有现成范式（钉 `[data-testid="error-block"]` 那种）；
3. **A 里的三条假绿**（驱动谎话 / 已移动 / 403 说成不存在）—— 它们**失效方向是绿**，最贵；
4. 其余 A（多为假红，会惩罚好文案，但不掩盖缺陷）。

---

# §4 纪律自查

- ✅ **没用 `git stash`**（PROTOCOL §15）
- ✅ **两次提交都带 pathspec**（`git commit -F <file> -- <paths>`），新文件先 `git add`；
  `git show --name-only` 复核 = **15 个，与预期逐条一致，无夹带**
- ✅ 没碰 `:10000`（pid 70802）、`/root/data-memo`、机器级指针；没用 `pkill -f`；没 `--amend`
- ✅ push 用具体 hash，push 后 `merge-base --is-ancestor` 复核通过
- ✅ 反向验证在 `/tmp/sf3` 隔离副本，仓库产物零改动
- ✅ **没碰别人的在飞文件**：`scripts/build-bundle.mjs`（prettier 报 warn，是别人的，**没格式化**）、
  `coordination/inbox/remediation.md`、`packages/runtime/src/selfcheck.ts` 全程未暂存
- ⚠️ 门禁那条红（`packStatus.ts :: isActivePack` 孤儿导出）**不是我的**，是 `a534e2e5…` 写到一半的

# §5 仍然挂着的 `[未验证]`

- **B11 修好后没在真浏览器里跑过**（按你的话保留，没抹平）。
- **13+5 里 15 处没有逐处反向验证**（见 §2 末）。
- 关键词清单里 **C 那 ~520 条是数量级估计，未逐条核对**；A/B/A+B 的 29 条都给了准确行号。
