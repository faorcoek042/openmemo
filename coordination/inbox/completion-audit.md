...
**第一句:门禁绿了。** `run 31316142174` `conclusion=success` —— 订正后的登记项
(`unpacking-percent-frozen`)跑绿,`e2e-browser` 不再挡发布。

**第二句:`downloading` 期间那几个数字**在走**,不是不动。**

## 实测数据(`run 31316526285`,windows-2025,真浏览器,只读 DOM)

装 `asr/whisper-large-v2-q8_0`(1.7 GB),每 200ms 采一次,**共 168 个采样点**:

| 观测项 | 取到的不同值 | 动没动 |
| --- | --- | --- |
| 百分比 | **88 种**(`0, 1, 3, 4, 5, 7, 9, 10 …`) | ✅ **在走** |
| 已下载字节 | **82 种**(`16 KB / 1.7 GB`→`18 MB / 1.7 GB`→`43 MB / 1.7 GB`→`66 MB / 1.7 GB`) | ✅ **在走** |
| 速度 | **31 种**(`46 KB/s`、`6.4 MB/s`、`2.5 MB/s`、`5.2 MB/s`) | ✅ **在走** |
| ETA | 0 种 | ⚠️ 见下,**不是结论** |

## ★ 完整文案序列 —— 问了六次,这次拿到了

```
["正在选择下载源", "下载中", "正在校验完整性", "正在安装"]
```

**四步都出现了,顺序正确,没有阶段倒退,没有 ASCII 枚举值。**
(「正在解压」这次没出现 —— 该模型是单权重文件,大概率**根本没有解包阶段**,
而不是没采到;`[未验证]`,与之前小包那次"采样太粗"是两回事。)

⇒ 也顺带回答了你让我留意的那条:**三平台文案不同就是采样随机性**,
不是路径差异 —— 窗口一拉长,序列立刻完整且正常。

## ⇒ 定案:用户那句"信息没有更新",只能是标题那一处

按你给的判据表:**数字在走 ⇒ 就是那个已修的文案缺陷(Toast 标题整个 `active` 期恒定)**。
下载过程中百分比/字节/速度**每 200ms 都在变**,`downloading` 那一段**没有藏别的东西**。

## ⚠️ 两处我**不能**据此下结论,如实说

1. **ETA 取到 0 种**,但这**不等于"ETA 不动"** —— 很可能是**我的正则没匹配上**
   (我按 `剩余|ETA` 抓,界面上未必是这两个词,或干脆没渲染 ETA)。
   **标 `[未验证]`,不写成"ETA 是死的"。**
2. **Toast 标题只取到 1 种:「有 3 项能力未就绪查看详情诊断」** ——
   那根本**不是任务 Toast**,是就绪横幅。**我的选择器抓错元素了**
   (`[data-testid="job-toaster"], [role="status"]` 命中了别的 `role="status"`)。
   ⇒ **B6b 至今仍然没有被真正验证过**:上次是空过,这次是抓错了对象。
   **这条我不声称已守。**

## 那次运行是红的,但**不是我的**

`run 31316526285` `failure` 来自 **`B11 ★ 移动失败时界面必须说话（目标文件夹已被删）`**
以及一条注入变异用的 `未捕获异常：boom`。**B11 不是我加的断言**,我没有碰它,
也没有把它算进我的结论 —— 报给你。

## 边界

- **未改产品代码。** D1 是一次性诊断,`--diagnose-download` 留空即整段跳过,
  **不可能拖慢门禁**;且表达式限定 `matrix.label == 'win32-x64'`,不烧另外两格的 1.7 GB。
- ⚠️ 我一度加了个 `onlyPlatform` 输入却没接线,**当场删掉** ——
  那正是 §14.1「声称已做、实则零实现」。
- 未碰 `:10000` / `/root/data-memo` / 机器级指针;未用 `pkill`;未建改删 release。
