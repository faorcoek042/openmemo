...
const TARGET = [{ target: 'Hugging Face', url: 'https://huggingface.co/api/models/ggerganov/whisper.cpp' }];
...
console.log('\n[1] no_proxy / 回环旁路（纯函数，不联网）');
{
  ...
  check('Ollama 所在的 127.0.0.1 旁路（本地 LLM 不能走远端代理）', shouldBypassProxy('127.0.0.1', []));
  ...
  const cfg = { ...DEFAULT_PROXY_CONFIG, mode: 'manual', httpsProxy: 'http://p:8080', noProxy: ['modelscope.cn'] };
  ...
  check('mode=off 一律直连', proxyUrlFor({ ...cfg, mode: 'off' }, 'https://huggingface.co/x') === null);
  const sys = { ...DEFAULT_PROXY_CONFIG, mode: 'system' };
  check(
    'mode=system 读环境变量',
    proxyUrlFor(sys, 'https://huggingface.co/x', { HTTPS_PROXY: 'http://env:3128' }) === 'http://env:3128',
  );
  check(
    'mode=system 也认环境里的 NO_PROXY',
    proxyUrlFor(sys, 'https://huggingface.co/x', { HTTPS_PROXY: 'http://env:3128', NO_PROXY: 'huggingface.co' }) === null,
  );
  check('SOCKS5 优先于 http 代理', proxyUrlFor({ ...cfg, socks5: 'socks5://s:1080' }, 'https://huggingface.co/x') === 'socks5://s:1080');
...
console.log('\n[4] 关键区分：代理挂了 ≠ 上游挂了');
{
  ...
  const cfg = { ...DEFAULT_PROXY_CONFIG, mode: 'manual', httpsProxy: `http://127.0.0.1:${deadPort}` };
  const rep = await testProxyConnectivity(cfg, { targets: TARGET, timeoutMs: 8000 });
  const p = rep.probes[0];
  check('代理不通时判定为 proxy_unreachable，而不是赖上游', p.result === 'proxy_unreachable', p.detail ?? '');
...
{
  const px = await startHttpProxy();
  applyProxyConfig({ ...DEFAULT_PROXY_CONFIG, mode: 'manual', httpsProxy: `http://127.0.0.1:${px.port}`, noProxy: [] });
  const before = px.state.tunnels.length;
  const res = await fetch('https://huggingface.co/api/models/ggerganov/whisper.cpp');
  await res.arrayBuffer().catch(() => undefined);
  check('未改一行调用点，全局 fetch 已走代理', px.state.tunnels.length > before, `隧道 ${px.state.tunnels.length - before} 条`);

  // 回环必须不经过代理，否则本地 daemon / Ollama 全断
  const loopTunnels = px.state.tunnels.filter((t) => t.startsWith('127.0.0.1') || t.startsWith('localhost'));
