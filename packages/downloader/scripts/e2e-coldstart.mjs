...
step(
  ...
  authed
    ? '已拿到 session cookie'
    : '未认证（注意：根路径 /#t= 有已知 bug，此处用 /models#t= 绕过）',
);
await shot('boot');
...
await shot('onboarding');
{
  ...
  step(
    '1',
    '首启引导可点着走',
    advanced > 0 ? 'yes' : obBody.length > 60 ? 'partial' : 'no',
    advanced > 0
      ? `推进了 ${advanced} 步（按钮：${[...seen].join('/')}）`
      : `页面有内容但点不动 · ${obBody.slice(0, 80)}`,
  );
...
await page.waitForTimeout(3000);
{
  ...
  const txt = hw
    ? (
        (await page.locator('[data-testid="runtime-hardware-card"]').first().textContent()) ?? ''
      ).replace(/\s+/g, ' ')
    : '';
  ...
  if (clicked) {
    ...
    step(
      ...
      '网页装加速后端（whisper.cpp）',
        ...
        ? `点「${clicked}」安装成功`
        : `点了「${clicked}」但未成功：${err || '任务未在 5 分钟内完成'}`,
    );
  } else {
    step(
      '3',
      ...
      n > 0 ? 'blocked' : 'no',
      n > 0
        ? `${packs} 个包，但没有可点的 whisper 包（本机无 GPU，且 whisper CPU 包可能不适用）`
        : `页面上 0 个可安装按钮（共 ${packs} 个包）`,
    );
  }
...
await page.waitForTimeout(3000);
{
  ...
  if (target) {
    ...
    for (let i = 0; i < 200; i++) {
      ...
      if (job) {
        if (job.totalBytes)
          pct = Math.max(pct, Math.round((job.completedBytes / job.totalBytes) * 100));
    ...
    step(
      ...
      '网页装 ASR 模型',
      ok ? 'yes' : 'no',
      ok ? `点「${label}」下载完成（${pct}%）` : `点了「${label}」但未成功 ${err}（到 ${pct}%）`,
    );
  } else {
...
/* ── 5. 装 VAD 模型 ── */
{
  ...
  step(
    ...
    vad.length > 0 ? 'yes' : 'no',
    vad.length > 0
      ? `${vad.length} 个：${vad.map((v) => v.id).join(', ')}`
      : '目录里没有 role=vad 的条目（daemon 可能未加载 models-asr-support.json）',
  );
}
...
/* ── 6. 装中文分词器（libsimple）── */
{
  ...
  step(
    '6',
    '网页装中文分词器 libsimple',
    installable.length > 0 ? 'yes' : 'blocked',
    ext.length === 0
...
await fs.writeFile(
  ...
  JSON.stringify(
    { ranAt: new Date().toISOString(), steps, pageErrors: [...new Set(pageErrors)].slice(0, 10) },
    null,
    2,
  ),
  'utf8',
