...
const ctx = await browser.newContext({ viewport: { width: 1400, height: 950 }, locale: 'zh-CN', acceptDownloads: true });
...
const bodyTxt = async () => ((await page.locator('body').textContent()) ?? '').replace(/\s+/g, ' ').trim();
...
rec('0', '打开网页并鉴权', (await ctx.cookies()).length > 0, `cookie=${(await ctx.cookies()).length > 0}`);
...
await page.waitForTimeout(3000);
{
  ...
  if (clicked) {
    for (let i = 0; i < 150; i++) {
      ...
      if (job && ['succeeded', 'failed', 'cancelled'].includes(job.state)) { ok = job.state === 'succeeded'; err = job.error?.messageZh ?? ''; break; }
    }
  }
  rec('2', '网页装加速后端', ok, ok ? `点「${clicked}」安装成功` : clicked ? `失败: ${err}` : `${total} 个按钮但无可点的 whisper 包`);
  await shot('backend');
}
...
await page.waitForTimeout(3000);
{
  ...
  if (await unhide.count()) { await unhide.click(); await page.waitForTimeout(900); }
  ...
  const ids = await page.evaluate(() =>
    [...document.querySelectorAll('[data-testid^="model-card-"]')].map((e) => e.getAttribute('data-testid')),
  );
  ...
  for (let i = 0; i < total; i++) {
    ...
    if (mb < best) { best = mb; target = btn; clicked = txt.slice(0, 40) + ' / ' + bt; }
  }
  let ok = false, err = '';
  if (target) {
  ...
  rec('3', '网页装 Whisper ASR 模型', ok, ok ? `点「${clicked}」下载完成` : clicked ? `失败: ${err}` : '找不到 Whisper 模型的下载按钮');
  await shot('model');
}
...
await page.waitForTimeout(2500);
{
  ...
  if (await urlIn.count()) {
    ...
    rec('4', 'probe 出标题/时长', pr.s === 200,
      pr.s === 200 ? `title="${pr.j?.title}" durationMs=${pr.j?.durationMs} adapter=${pr.j?.adapterId} mediaCount=${pr.j?.mediaCount} · requiresAuth=${pr.j?.requiresAuth}（=“未知”，非“不需要登录”）`
  ...
  if (await go.count()) {
    ...
    for (let i = 0; i < 150; i++) {
      ...
      if (cand) {
        ...
        if (segs.length > 0) { realText = segs.map((s) => s.text).join(' ').trim(); break; }
        if (cand.status === 'failed') break;
      }
      if (i === 5) await shot('transcribing');
    }
    rec('5', '导入 → 转写 → 出现真实文字', realText.length > 0,
      realText.length > 0 ? `note=${noteUid} status=${status} 文字="${realText.slice(0, 160)}"` : `note=${noteUid ?? '未创建'} status=${status || 'n/a'} 无转写文字`);
...
if (noteUid && realText) {
  ...
  await page.waitForTimeout(3000);
  /* eslint-disable-next-line no-undef -- runs inside the browser via page.evaluate() */
  const genBtns = await page.evaluate(() => [...document.querySelectorAll('button')].map((b) => (b.textContent || '').trim()).filter(Boolean).slice(0, 30));
  console.log(`        详情页按钮: ${JSON.stringify(genBtns)}`);
  const gen = page.locator('button:has-text("思维导图"), button:has-text("生成导图"), button:has-text("导图")').first();
  let genOk = false;
  if (await gen.count()) {
    await gen.click();
    await page.waitForTimeout(3000);
    for (let i = 0; i < 60; i++) {
      ...
      if (mm.s === 200 && mm.j && (mm.j.nodes?.length || mm.j.root)) { genOk = true; break; }
    }
  }
  ...
  if (container > 0 && nodes > 0) {
    ...
    if (box) {
      ...
      await page.mouse.down(); await page.mouse.move(box.x + 130, box.y + 80, { steps: 10 }); await page.mouse.up();
      await page.waitForTimeout(800);
      await node.click({ button: 'right' }).catch(() => {});
      await page.waitForTimeout(1000);
      menu = (await page.locator('[role="menu"], .context-menu, .mind-elixir-ctxmenu, menu').count()) > 0;
      await page.keyboard.press('Escape').catch(() => {});
      await page.keyboard.press('Control+z').catch(() => {});
      await page.waitForTimeout(600);
    }
    rec('7', '导图拖拽/右键/撤销', menu ? true : 'part', `容器${container} 节点${nodes} · 拖拽已执行 · 右键菜单${menu ? '出现' : '未出现'}`);
  } else {
  ...
  if (await exp.count()) {
    ...
    await exp.click(); await page.waitForTimeout(900);
    ...
    if (dl) {
      ...
      rec('8', '导图 SVG/PNG 导出', t.includes('<svg'), `${(t.length / 1024).toFixed(1)} KB，${t.includes('<svg') ? '合法 SVG' : '非 SVG'}`);
...
await fs.writeFile(path.join(SHOTS, 'report.json'), JSON.stringify({ ranAt: new Date().toISOString(), base: BASE, audio: AUDIO, noteUid, realText, results: R, pageErrors: [...new Set(errs)].slice(0, 10) }, null, 2), 'utf8');
