...
| **`OPENMEMO_AUTH=none` 的契约字段**          | 🟡 已生效，契约未定，**前端不再有任何判断点** | `oss-scout` → `architect`  | `GET /api/health` **至今没有 `auth` 字段** `[实测]`，这半句仍然成立。⚠️ **订正（本轮）**：后半句"前端 `lib/api/auth-mode.ts` 用『宽容读』顶着"**是假的** —— ① 那个文件已被 **PR #83（`8378a1c`）整个删掉**（6 个导出零消费方）；② 它在被删之前**也没有在顶**：`setAuthMode()` 全仓零调用，模块内的 `mode` 永远停在 `'unknown'`，而整个模块零 importer（`coordination/inbox/debt-audit.md` C12 `[实测]`）。**今天真正管这件事的是 daemon 侧的不变量**：`apps/daemon/src/http/server.ts` 的 `POST /api/auth/session` 里 `if (!authRequired())` 直接回 200 + cookie + CSRF + `authMode: 'none'` —— 理由写在那段注释里：不变量应该是"鉴权关了握手就不该失败"，而不是"每个调用方都要记得判断鉴权关没关"。前端 `lib/api/connect.ts` 因此**不读任何鉴权模式**，两种模式走同一条流程。**还欠的是**：`/api/health` 的 `auth` 字段仍未定稿，且握手回执里那个 `authMode` 今天前端零消费（诊断页想显示"这是免鉴权握手"的话得自己接） |
...
> - **代理覆盖每一条出网路径** —— `scripts/ci/proxy-coverage-audit.mjs` 逐条实测过，
>   但它**是手动脚本，当前没有任何自动调用方**（`[实测 2026-08-11]` 全仓零执行引用）。
>   所以这条是**一次性实测结论，不是持续保证**：今天改坏了代理，没有任何东西会红。
