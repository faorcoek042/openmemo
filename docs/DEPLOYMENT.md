...
| **Windows + AMD（DirectML）** | 🔴 **DirectML 无产物。** Windows 上的 AMD 加速走 Vulkan 那条（`whispercpp-vulkan-win-x64` 已在 `backends.json` 里），不走 DirectML。⚠️ 但**没有人在真 AMD 卡上验过**，标 `[未验证]`。                                   |
...
`openmemo-probe` **随每个 whisper 包一起出厂**（T-167，例外写在 `ADR-015 §7`）——
它必须与 ggml 后端模块同目录才跑得起来，所以只能由我们放进包里。
...
| #   | 功能                                                   | 状态                                                                                                                                                                                                                                                       |
| --- | ------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| F1  | 音视频**链接**导入（YouTube / Bilibili / 播客 / 直链） | yt-dlp 随产品下载并校验，`[CI 实测]` 三平台都装上了。全链路（真下载 → 转写）在 2026-08-02 被 `gpu-runtime` 在这台机器上真跑过一次（`adapter: yt-dlp \| downloaded 252182 bytes`）—— `e2e-import` 腿现在三平台每次都真跑（用预编译包 + 真实 HTTP，见 §1.5） |
...
> **跑 `pnpm license:report` 时该看到什么**（2026-08-09 本机实测）：
> C 类 109 条、**0 条 UNKNOWN**（`llm-providers.json` 的 24 个远程 API 服务商按 `N/A` 单列，
> 许可证概念不适用，不计入）；B 类（npm 依赖）**268 条**。
>
> ⚠️ 若 B 类跑出来是**空的**，看 stdout 里 `packagesError` 那一行的具体报错
> （多半是 `pnpm licenses list` 在你的环境里失败了）——**空的不是常态**。
