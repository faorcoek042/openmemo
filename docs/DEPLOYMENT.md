...
> ★ **2026-08-08 起这一节分成两条路。** 此前它只描述「克隆源码自己构建」那一条 ——
> 因为当时**没有预编译包**。v0.2.0 起有了，于是下表里 Node / pnpm 那两行
> **对绝大多数用户不再适用**。订正记录见 §1.1.1。

#### 路线 A：下预编译包（推荐）——**不需要 Node、不需要 pnpm、不需要 git**

| 项       | 要求                                                                                | 出处                                     |
| -------- | ----------------------------------------------------------------------------------- | ---------------------------------------- |
| Node.js  | **不需要**。包里自带官方 Node 运行时（`runtime/node`，钉死 v22.23.1）               | `[CI 实测]` `scripts/build-bundle.mjs`   |
| pnpm     | **不需要**                                                                          | 同上                                     |
| git      | **不需要**                                                                          | 同上                                     |
| 系统     | Linux x64（glibc ≥ 2.34）· Windows x64（**需 VC++ 2015-2022 运行时**）· macOS arm64 | §1.3                                     |
| 首次运行 | **系统会拦一次**：Windows 走 SmartScreen「仍要运行」；**macOS 必须右键 →「打开」**  | §8.9（这一条是 v0.2.0 最主要的翻车原因） |
| 磁盘     | 包解开后 132–192 MiB，**外加**组件与模型（见下表"磁盘"一行）                        | §1.3 / README 平台表                     |
...
| `whispercpp-vulkan-linux-x64`（**已重建**） | ubuntu-24.04 + glibc 2.35 容器 | **2.34**        | ✅ Ubuntu 22.04 / Debian 12 都能跑 |
...
| **Windows + AMD（DirectML）** | 🔴 **DirectML 无产物**（这半仍然为真）。~~Vulkan 那条我们自己能编出来，但同样卡在 §8.4，**未接入**。~~ **← 2026-08-08 订正（`docs-sync`，§13）：`whispercpp-vulkan-win-x64` 已在 `backends.json` 里**（`[实测]` 14 条目录里有它）。Windows 上的 AMD 加速走 Vulkan 那条，不走 DirectML。⚠️ 但**没有人在真 AMD 卡上验过**，标 `[未验证]`。 |
| **macOS Intel（x64）**        | ✂ 用户 2026-08-05 明确裁掉，不构建。                                                                                                                                                                                                                                                                                                     |
| **linux-arm64**               | ✂ 同上裁掉。（讽刺的一条：它那两个构建腿曾是唯二全绿的 Linux job，删的不是坏的那些，是用不到的那些。）                                                                                                                                                                                                                                   |

🔴 **Windows + NVIDIA CUDA 今天仍然装不上，但理由已经不是下面这一条了。**

~~包（`whispercpp-cuda-12.4-win-x64`）在目录里，但它是 L2、需要硬件探针门控，
而 **`openmemo-probe` 至今没有分发通道**（CI 会构建它，没有任何 manifest 提供下载地址；
`grep openmemo-probe vendor/manifests/*.json` → 0 命中）。~~

> **订正（2026-08-08，`docs-sync`，依据 `PROTOCOL §13`）**：
> **`openmemo-probe` 现在有分发通道了。** T-167（2026-08-07，Manager 裁定，
> 例外写在 `ADR-015 §7`）起，探针**随每个 whisper 包一起出厂** ——
> 它必须与 ggml 后端模块同目录才跑得起来（裸跑报 `cannot open shared object file:
libggml-base.so.0`），所以只能由我们放进包里。
> `[实测 2026-08-08]` 同一条命令今天是 **10 命中**（`vendor/manifests/components.json`
> 的 provenance 说明 + `backends.json` 的 `providesFiles`），**不是 0**。
>
> ⚠️ **但不要据此以为 CUDA 能装了**：探针要**先装一个 whisper 包**才会落到盘上，
> 而在那之前 L2 仍然不适用。**这一格今天的真实状态标 `[未验证]`** ——
> 本轮没有在真 N 卡上验过（见下方"仍然验不了的"）。

下面这三条证据**仍然成立**，只是它们描述的是"探针还没落盘时"的状态：

`[本机实测]` `GET /api/runtime/hardware` 里 cuda/vulkan/rocm/metal 四条全是
`probe did not complete: probe executable not found`（这台开发机上没装过任何 whisper 包）；
...
#### 🔴 仍然验不了的（**验不了 ≠ 没做**，不许把它写成"不支持"）

- **Windows + CUDA 到底能不能用：`[未验证]`，而且我们没有办法验。**
  GitHub 托管 runner **没有 N 卡**，所以「装上之后是不是真的调用了 GPU」
  这个问题**在本项目现有的验证能力之内是无解的**。
  包能编出来、能下载、能校验；**真 GPU 上到底加不加速，`UNKNOWN`。**
- **真 GPU 上的加速比：`UNKNOWN`。** 同上，没有硬件。
  README 平台表里 Vulkan 那格写"没在真 GPU 上验过"就是这个意思。
- `windows-2022/cuda` 这条构建腿**为什么保留**：`UNKNOWN` ——
  追查过但没找到记录（见 `docs/design/D-11-ci-platform-facts.md`）。
  **刻意保留 `UNKNOWN`，不编一个理由上去。**

### 1.4 五个基础功能今天的真实状态（章程 F1–F5）

| #   | 功能                                                   | 状态                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| --- | ------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| F1  | 音视频**链接**导入（YouTube / Bilibili / 播客 / 直链） | yt-dlp 随产品下载并校验，`[CI 实测]` 三平台都装上了。全链路（真下载 → 转写）在 2026-08-02 被 `gpu-runtime` 在这台机器上真跑过一次（`adapter: yt-dlp \| downloaded 252182 bytes`）—— 这条是 `[报告]`，~~**没有人独立复核，CI 也不跑它**（会打真实站点）~~ **← 2026-08-08 起不再为真：`e2e-import` 腿现在三平台每次都真跑**（用预编译包 + 真实 HTTP，见 §1.5）。同一轮还查出并修掉了「yt-dlp 命中 `--max-downloads` 返回 101 被当成失败」这个让 F1 对每个视频都失败的 bug |
...
#### 🔴 Windows 上「移动」可能其实是「复制了一份，源还在」（2026-08-08 新增）

`[CI 实测 2026-08-08 run 31250730491，windows-2025]` 走复制路径时，最后那步
`fs.rm(源目录)` **会失败** —— Windows 删不掉仍被 daemon 打开的 `openmemo.db`。

**此前的后果**：调用方拿到的仍是 `ok:true`，界面照旧说**「已移动 N 个文件到新位置」**，
而数据**被复制了一份留在原地**，里面包含**明文的 `secrets.json`（你的第三方 API Key）**。

> 那句话不实，而且是最危险的一种：**用户据此以为旧位置已经空了。**

**已修（v0.3.0）**：`sourceRemoved` 现在是一个独立的结构化字段，
`false` 时界面**必须改口，不许再说"已移动"**。判据不是"让 Windows 也用 rename"
（跨卷 rename 本来就会失败，复制是必要的退路），而是**"界面说的和实际发生的必须一致"**。

**你要做的**：如果界面告诉你源目录没能删掉，**手动去旧位置确认并删除** ——
尤其是 `secrets.json`。停掉 daemon 之后再删就能删掉了。
...
**← 2026-08-08 订正（`docs-sync`，依据 §13）**：`[实测]` `backends.json` 今天 14 条，
**含 `whispercpp-vulkan-linux-x64` / `whispercpp-vulkan-win-x64` / `whispercpp-cuda-12.4-win-x64`**。
它们进目录的前提是 D-11 §8.4 那三条阻碍被逐条消掉 —— 关键一条是**包改成自包含**
（`providesFiles` 23 项含 `whisper-cli` 与后端模块），**不再依赖别的包目录里的 `ggml-base`**。
换句话说：**§8.4 描述的那个结构性问题是被修掉了，不是被绕开了。**

⚠️ **但"装了没变快"这个症状本身仍然可能发生**，因为 `GGML_BACKEND_DL=ON` 下
`dlopen` 失败依旧不算错误。而且**真 GPU 上到底加不加速，我们验不了**（§1.3 那条 `UNKNOWN`）。
macOS 的 Metal 跟核心包一起出厂，是这一族里唯一有实测证据的一格。

### 8.9 🔴 下了预编译包，双击**什么都没发生** / 弹出报错（2026-08-08 新增）

**这是 v0.2.0 最主要的翻车原因**，三个平台各不相同。用户 2026-08-08 的原话：
「Windows 上双击运行提示出错，mac 上被安全性各种拦截，也没有窗口打开。」

#### macOS：双击 `OpenMemo.command` 没反应 —— **不是包坏了，是 Gatekeeper 拦在最前面**

`[CI 实测 run 31248222725]`

```
$ spctl -a -vvv -t open --context context:primary-signature OpenMemo.command
  rejected
  source=no usable signature      ← 拦的是我们自己未签名的脚本
$ open OpenMemo.command
  ETIMEDOUT，40 秒内界面从未起来
```

**拦的是脚本本身，不是 Node** —— 包里的 `runtime/node` 有
`Developer ID Application: Node.js Foundation (HX7739G8FX)` 正式签名，
带着隔离属性**照样跑得通**（实测 `node-ran-ok`）。
所以你看到的是「**终端窗口根本没打开**」，既没有界面也没有任何错误信息。

**怎么过**：**右键点 `OpenMemo.command` →「打开」→ 弹窗里再点「打开」**，只需一次。
（macOS 15+ 可能要去「系统设置 → 隐私与安全性」，在下方点「仍要打开」。）

⚠️ **换命令行 `tar xzf` 解压绕不开它。**
`[CI 实测]` 归档带 `com.apple.quarantine` 时，访达的「归档实用工具」与命令行 `tar`
**都会**把该属性传给解出来的每一个文件（同一个值）。
~~v0.2.0 的说明里写过「用命令行解压就不会被拦」~~ —— **那句话是错的**，已订正
（`docs/design/D-17-prebuilt-bundles.md` §5.1）。

另一条路是 `xattr -dr com.apple.quarantine "<解压出来的文件夹>"`。
⚠️ 它**等于对这些文件关掉 Gatekeeper 检查**，请确认你信任来源再执行。

> 💡 包里那份 `READ-ME-FIRST.txt` **不会被拦**（`[CI 实测]` 带着隔离属性仍可完整读出）
> —— 被挡住的时候先读它。

#### Windows：双击 `start.cmd` 弹出 `'m' is not recognized as an internal or external command`

**v0.3.0 已修。** 成因是启动脚本里有中文注释，而 `cmd.exe` 按**控制台 OEM 代码页**
解析 `.cmd`（中文系统是 936）—— GBK 解码器错位配对，把 `rem` 的 `r`/`e` 吃掉，
漏出一个 `m` 当命令执行。`[CI 实测]` 同一个包在 437 下没有这一行，936 下有。

现在 `start.cmd` 是**纯 ASCII**（组装时逐字节校验，非 ASCII 当场失败），
并且结尾有 `pause` —— **出错信息不会再随窗口一闪而过**。

还会遇到的是 **SmartScreen「Windows 已保护你的电脑」**（我们不买代码签名证书）：
点「更多信息」→「仍要运行」。

⚠️ **不要在压缩包里直接双击 `start.cmd`** —— Windows 只解出你点的那一个文件，
其余全都不在，必然失败。先右键「全部解压缩」，再进解压出来的文件夹双击。
（v0.3.0 的启动脚本会**明确告诉你**这一点，而不是闪一下就关。）

#### Linux：双击 `start.sh` 只是用文本编辑器打开了它

正常现象 —— GNOME Files 等文件管理器默认**不执行**脚本。
**在终端里跑**：`./start.sh`。包里没有 `.desktop` 入口。
...
**本仓库**：`package.json` 仍写 **`UNLICENSED`**（个人自用项目，ADR-002 v2：
用户已明确"仅个人/自用，不追求商用、不上应用商店"），根目录另有 `LICENSE` 说明授权状态。
