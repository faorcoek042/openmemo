...
> whisper）。转写那条路的证据**全部来自 CI**，本文不会把它写成本机实测。
> ~~证据全部来自 CI run 31076010999~~ **← 2026-08-08 订正（`docs-sync`，§13）：
> 今天不止那一轮了** —— 还有 `cold-start-audit` **run 31167151669**（§1.3 的重核来源）
> 与 F1–F5 四条 e2e 腿（§1.5）。**每条结论的 run id 写在它自己那一行**，以那里为准。
...
> 这一节此前只描述"克隆源码"那条路（预编译包出现之前写的）。订正原文与经过见
...
> 这一格此前写着「vulkan-linux 需 GLIBC 2.38，Ubuntu 22.04 上加载失败」且「不在目录里」——
> **两句今天都已不为真**（包已重建到 ≤2.34，且已进目录）。订正原文见
> [`D-19`](design/D-19-user-doc-provenance.md)。
...
| 平台                                   | 能用的后端                           | 转写实测（同一段音频）     |
| -------------------------------------- | ------------------------------------ | -------------------------- |
| **Linux x64**                          | CPU + Vulkan（Vulkan 包装得上）      | 2.1s                       |
| **Windows x64**                        | 只有 CPU（CUDA 见下）                | 4.0s                       |
...
F1–F5 各有一条端到端腿（用预编译包、走真实 HTTP、三平台各跑一次），外加一条
"人能不能双击打开"的腿。**一批包没跑过这些腿就发不出去** —— 闸门在发布流程里，不靠人记得。

> ⚠️ **它们证明不了的**：托管 runner 没有 N 卡，**真 GPU 上到底加不加速仍然是 `UNKNOWN`**（§1.3）。
> 腿多不等于覆盖全。
>
> 具体每条腿覆盖什么、最近一次绿的 run 号，见
> [`docs/design/D-19-user-doc-provenance.md`](design/D-19-user-doc-provenance.md)。
...
### 8.9 🔴 下了预编译包，双击**什么都没发生** / 弹出报错

**这是 v0.2.0 最主要的翻车原因**，三个平台各不相同。

#### macOS：双击 `OpenMemo.command` 没反应 —— 不是包坏了，是 Gatekeeper 拦在最前面

Gatekeeper 拦的是**我们自己未签名的那个启动脚本**（不是 Node，包里的 `node` 有官方
Developer ID 签名）。它拦在**打开脚本**那一步，所以**终端窗口根本没被启动** ——
你既看不到界面，也看不到任何错误。

**怎么过**：**右键点 `OpenMemo.command` →「打开」→ 弹窗里再点「打开」**，只需一次。
（macOS 15+ 可能要去「系统设置 → 隐私与安全性」，在下方点「仍要打开」。）

⚠️ **换命令行 `tar xzf` 解压绕不开它** —— 访达的「归档实用工具」与命令行 `tar`
**都会**把隔离属性传给解出来的每个文件。（v0.2.0 曾写反过这一条，订正见
[`D-17`](design/D-17-prebuilt-bundles.md) §5.1。）

另一条路是 `xattr -dr com.apple.quarantine "<解压出来的文件夹>"` ——
⚠️ 它**等于对这些文件关掉 Gatekeeper 检查**，先确认你信任来源。

> 💡 包里那份 `READ-ME-FIRST.txt` **不会被拦**，被挡住时先读它。
> 实测原文（`spctl` 输出、run 号）见 [`D-18`](design/D-18-launch-experience.md) §3。

#### Windows：双击 `start.cmd` 弹出 `'m' is not recognized as an internal or external command`

**v0.3.0 已修**（启动脚本改成纯 ASCII，并在出错时 `pause`，报错不再一闪而过）。
仍会遇到 **SmartScreen「Windows 已保护你的电脑」**：点「更多信息」→「仍要运行」。
...
正常现象 —— 文件管理器默认不执行脚本。**在终端里跑**：`./start.sh`。

### 8.10 🚧 三条**已知但成因未定**的首次使用故障（占位，等实测）

> ⛔ **本节现在只有位置，没有内容 —— 这是故意的。**
>
> 用户 2026-08-08 在真机上报了下面三条，**三条的成因都还没定案**，
> 各有一路正在查。**在实测结论出来之前，这里不写处置办法** ——
> 写了就是猜，而一条猜出来的排查步骤比没有更糟：
> 用户会照着做、做完没用，然后认为自己已经排除了这个原因
> （本仓刚在 macOS 的 `tar` 那条建议上吃过一模一样的亏，见 §8.9）。
>
> **等对应那条修复落地后，由那一路把实测结论填进来。**

| 症状（用户原话）               | 状态                   | 谁在查 / 落地后补什么                                                                  |
| ------------------------------ | ---------------------- | -------------------------------------------------------------------------------------- |
| **六个后端全报「探针找不到」** | 成因排查中，`[未验证]` | "探针进包"那条修复。落地后补：探针到底装在哪、为什么最小包里没有、用户该怎么确认它在了 |
| **空数据目录下装不了任何组件** | 成因排查中，`[未验证]` | "最小包冷启动"那条。落地后补：冷装第一步到底卡在哪、有没有手工绕法                     |
| **点按钮完全没反应**           | 成因排查中，`[未验证]` | "真浏览器 e2e"那条。落地后补：哪些按钮、什么条件下复现、控制台有没有报错               |

⚠️ **给填这一节的人**：请照 §8.9 的形状写 —— **先给实测原文（报错原话 / 日志），
再给处置**。只有"怎么办"没有"你会看到什么"的排查条目，用户对不上号。
...
> 这一段此前写着「根目录没有 LICENSE 文件」与「要分发就得先自建 LGPL 版 FFmpeg」——
> **两句都已不为真**。订正原文与依据见
> [`docs/design/D-19-user-doc-provenance.md`](design/D-19-user-doc-provenance.md)。
...
>
> ① ~~`license-report.md` 里 C 类那 7 行显示 `UNKNOWN`，那是按清单文件粒度统计的结果，
> 不是说这些组件没有许可证~~ —— 根因是 `collectManifests()` 从未下钻每份 manifest 的
> `.packs[] / .components[] / .models[] / .providers[]` 数组，`Array.isArray(json) ? json : [json]`
> 这行对着**顶层是对象、不是数组**的 7 份文件，永远把整份文件当一条假 item，逐条 `.license`
> 自然读不到，恒定 fallback 成 `UNKNOWN`——这不是"统计口径粗"，是**没有一次真正读进去过**。
> 已重写为按文件登记表逐条下钻并读取真实字段；2026-08-09 本机实测：C 类 109 条，
> **0 条 UNKNOWN**（`llm-providers.json` 的 24 个远程 API 服务商条目按 `N/A` 单列，
> 不是许可证概念适用但缺填，不计入这个 0）。脚本新增一条守卫：C 类 UNKNOWN 占比超过
> 10% 就 `exit 1`，防止这个坑再次悄悄出现却没人发现。
>
> ② 报告里的 B 类（npm 依赖）—— 2026-08-09 本机实测跑一次是 **268 条，不是空的**；
> 若你跑出来是空的，看 stdout 里 `packagesError` 那一行的具体报错（多半是 `pnpm licenses
list` 在你的环境里失败了），**不要默认"B 类是空的"就是常态**，那本身也是一次事实缺口，
> 不是设计如此。
