...
## 附录 Z：Workspace 的最终裁决（2026-08-08，用户本人）

**不做。正式关掉这条。**

本文附录 A-1 曾把「空间管理 = Workspace，一等公民，且与磁盘目录绑定」列为竞品的
一等特性并请求 Manager 复议 —— ADR-006 当年已决定不做，此后这条**每隔几轮就被
重新翻出来问一次**，而每次都没有得到答复，于是它既没被做也没被关掉。

**2026-08-08 用户本人的裁决：不做。**

依据（用户与既有约束）：

1. **不在 F1–F5 里。** 章程的验收基准不含空间/项目分组。
2. **与已裁项同族。** 用户此前已明确裁掉 TTS、移动端 App、团队协作、云端账号、
   支付、macOS Intel、linux-arm64、ROCm —— 都是"扩张产品边界"而非"让现有功能可用"。
3. **边际收益低。** 笔记侧已有文件夹筛选 + 全文检索（含中文分词），
   再叠一层分组解决的问题，今天已有两条路可以解决。
4. **完整版会牵动最贵的地方。** "每空间独立磁盘根"会动到数据目录指针与单实例逻辑 ——
   而那两处正是本仓踩过坑的地方（指针是机器级单例、搬家会静默弄坏符号链接）。

**本条从此不再是"待裁决"，是"已裁决：不做"。**
下一个人如果想重开它，请带着**用户的新指令**来，而不是带着"竞品有"来 ——
**「它有我们没有」不等于「我们该有」**，这是本轮对照的既定判据。

已同步写入 `README.md` 的「不做」清单。
