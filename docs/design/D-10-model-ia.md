...
> - **D2（`role=llm` 下架范围）仍成立**：`vendor/manifests/models-llm.json` **今天仍在**（5 条），
>   而 `http/rest/manifests.ts` 已改成**列目录加载所有 `*.json`**（不再写死文件名），
>   所以它会被加载。"停用 manifest 还是只隐藏 Tab"这个范围问题**没有人裁过**。
> - **D3（一级导航「运行时」是否改名）仍成立**：`zh-CN.json` 的 `nav.runtime` 今天仍是 `"运行时"`。
