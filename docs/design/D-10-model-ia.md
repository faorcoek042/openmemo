...
> 建议一并露出~~ → **已闭合，而且这条建议当时就是多余的**：`showModel` 的默认值一直是
> `true`，所以那两处"未传"恰恰等于**传了 true**，选择器从来都是露出来的。
> 这条建议读起来像"有两个页面缺功能"，实际一个都不缺 —— 它盯的是 prop 有没有出现在
> JSX 上，而不是那个 prop 生效后的值。`showModel` 现已整个删除（三个调用点全取默认值，
> `false` 分支不可达），所以这条也不再有对象。
...
> **✅ 逐条复核（2026-08-08，由 `e2e-runtime` 就地改，依据 PROTOCOL §13）**
>
> **D1 已闭合，D2 / D3 仍然成立。**
>
> - **D1（`anthropic` vs `claude` 会静默走错分支）—— 已修，按本表建议的方式修的。** `[实测]`
>   `apps/daemon/src/llm/resolve.ts` 今天**先读目录的 `kind` 字段**分派
>   （`:76-78` 取 `rec.kind`，命中 `anthropic` / `gemini` / `openai-compatible`），
>   硬编码 id 只作**老配置兜底**，而且那条兜底**两个 id 都认**
>   （`:86` `providerId === 'anthropic' || providerId === 'claude'`）。
>   建议 ① 与 ③ 都落地了 ⇒ 「P1 一上线就坏」这个风险不存在了。
>   ⚠️ 建议 ②（让 daemon 读 `llm.providers` 成为唯一清单）**是否落地未核** `[未验证]`。
> - ~~**D2（`role=llm` 下架范围）仍成立**~~ → **已裁并已执行（2026-08-08）**：
>   Manager 裁定**关掉**那个 API 面。做法是服务端 `withoutRetiredRoles()`
>   （`http/rest/state.ts`，在 `loadModelCatalog()` 之后摘掉 `role=llm`），
>   一处收口同时堵住 `/api/models/catalog` 与 `findCatalogModel()`（`pull` 的入口）；
>   **`models-llm.json` 刻意保留**（删掉会让下一个人把它加回来），
>   历史写在 `ADR-016` 决策 3 的「执行补记」里。
>   本条建议的"保留类型联合里的 `llm`"**照办了** —— `active.llm`、`MISSING_LLM`、
>   `gpu.llm` 全部未动，关的只是**下载入口**。
> - ~~**D3（一级导航「运行时」是否改名）仍成立**~~ → **已裁并已执行（2026-08-08）**：
>   Manager 裁定**改成「本机组件」，路由 `/runtime` 不动**（URL 是可分享物，
>   而且 README / DEPLOYMENT / 各处文档都写着 `/runtime`，改了会把一堆文档变成不实）。
>   本条当时说"我不改他人交付物"是对的；现在由 Manager 裁完后统一改。
>   ⚠️ 实际改动**不止 `nav.runtime` 一处**：面向用户提到这个页面的地方一共
>   **11 处**（locale 5 + `packages/runtime/src/selfcheck.ts` 的补救文案 7 处里指向该页的全部），
