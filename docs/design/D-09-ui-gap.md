...
>
> 这一节整体挂着"待裁决"，但 6 条里**只有 3 条今天还成立**。逐条核过的结果：
>
> | #   | 今天                                                                                                                                                                                                                                                               |
> | --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
> | 1   | **已闭合** —— `components/common/HealthBanner.tsx` **文件已不存在**，无需再裁"由谁清理"                                                                                                                                                                            |
> | 2   | **仍成立** —— `Settings.routes.tsx` 今天是 `settings/:section → <SettingsPage />`，五个 section 仍渲染同一页（`settings` 裸路径已加 `Navigate` 到 `/settings/general`，但这只是补了入口，没有拆页）                                                                |
> | 3   | **已闭合**（2026-08-09 由 `check-pending-claims` 的谓词查出）—— 三个 `<option>` 今天都走 `t()`（`SettingsPage.tsx:52-54`），代码里还留了一段注释说明当初为什么改。此前这一行一直写着「仍成立」，**而代码早就改了、没人回来更新这张表** —— 这正是那个脚本要防的形状 |
> | 4   | **大部分已闭合** —— `en.json` 今天 `models` 136 条 / `runtime` 72 条词条，两页分别有 39 / 15 处 `t()` 调用。⚠️ 两个文件里仍有中文，但抽样看**主要是注释**；「UI 正文是否 100% 走 i18n」`[未验证]`                                                                  |
> | 5   | 纯知会，无需裁决                                                                                                                                                                                                                                                   |
> | 6   | **仍成立** `[未验证]` —— 没有证据表明 `verifying` / `blocked` 两态被真实触发过                                                                                                                                                                                     |
>
> 也就是说：**#1 可以划掉，#2 #3 #6 仍然要人处理，#4 剩个尾巴。**
> 保留原文不删（§13：「此前写着什么」是可追溯性的一部分）。
