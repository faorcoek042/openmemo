...
### 3.2-bis ★ 说明书自己会不会也被拦（`[CI 实测 run 31249135458]`）

整份修复的支点。**如果 `READ-ME-FIRST.txt` 同样打不开，我只是把解法从一扇锁着的门
搬到了另一扇锁着的门后面。** 所以这一条必须实测，不能推断"纯文本应该不会被拦"。

同一棵树、同一个 quarantine 值下的对照：
...
**结论：说明书不会被拦。** 用户在被 Gatekeeper 挡住的那一刻，
**读得到那份告诉他怎么办的文件** —— 裁决 (A) 的落地前提成立。

> ⚠️ **顺带量出一条重要的方法论**：**`spctl` 的判决不能用来预测"文档能不能打开"。**
> 它对 `.txt` 也答 `rejected` —— 因为它回答的是「**当作代码执行**」的策略，
> 而不是「**当作文档打开**」的策略。同一个判决、同一个属性，两个相反的实际结果。
> 拿 `spctl` 当唯一判据会得出"说明书也打不开"的错误结论。
> **判据必须是"用户到底读不读得到这些字节"，不是某个工具怎么说。**

`UNKNOWN`：GUI 层「TextEdit 有没有真的弹出来、有没有额外弹窗」在无头 runner 上取不到
（没有交互会话）。这里给的是**字节可读性** + **`open` 的返回值** + **与 `.command` 的对照**。
