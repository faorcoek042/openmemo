import { a as api, f as qk, r as formatPercent } from "./ErrorBlock-yofocnTu.js";
import { n as useProgressStore } from "./progress.store-CB1TnKEq.js";
import { a as ProgressMeter, t as approxEta } from "./time-Dn1EgsA-.js";
...
function useTranscriptQuery(uid) {
	return useQuery({
		...
		queryFn: () => api(`/notes/${uid}/transcript`),
		enabled: Boolean(uid)
	});
}
/** probe：秒级返回，**先于下载**。让"认对了没有"和"需要登录"都提前暴露（D-01 §5 F1）。 */
function useProbeMutation() {
	return useMutation({ mutationFn: (url) => api("/import/probe", {
		method: "POST",
		body: { url }
	}) });
}
