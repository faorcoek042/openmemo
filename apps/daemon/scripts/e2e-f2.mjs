...
console.log(
  `[1] 换取 session: HTTP ${sessRes.status}  cookie=${sid ? '✅' : '❌'}  csrf=${csrf ? '✅' : '❌'}`,
);
...
for (const e of events.slice(0, 12)) {
  ...
  else if (e.event === 'transcribe.segment')
    extra = `seq=${d.seq} ${d.startSec}s "${(d.text ?? '').trim().slice(0, 42)}"`;
...
if (seg.length > 3) {
  ...
  for (const e of seg.slice(-2)) {
    console.log(
      `      transcribe.segment  seq=${e.data.seq} ${e.data.startSec}s "${e.data.text.trim().slice(0, 42)}"`,
    );
  }
}
for (const e of events.filter((x) =>
  ['transcribe.done', 'job.done', 'note.updated'].includes(x.event),
)) {
...
const trRes = await fetch(`${base}/api/notes/${imp.noteUid}/transcript`, {
...
for (const s of tr.segments.slice(0, 5)) {
  console.log(
    `      [${String(s.startMs).padStart(6)}-${String(s.endMs).padStart(6)}] ${s.text.trim().slice(0, 60)}`,
  );
}
...
if (audioAsset) {
  const full = await fetch(`${base}${audioAsset.url}`, {
    method: 'HEAD',
    headers: { Cookie: cookie },
  });
  ...
  console.log(`\n[7] /media Range 字节流`);
  console.log(
    `      HEAD  → ${full.status}  Content-Length=${full.headers.get('content-length')}  Accept-Ranges=${full.headers.get('accept-ranges')}`,
  );
  console.log(
    `      Range → ${rng.status}  Content-Range=${rng.headers.get('content-range')}  实收 ${buf.byteLength} 字节`,
  );
...
for (const [type, fields] of Object.entries(REQUIRED)) {
  ...
  if (!sample) {
    console.log(`      ${type.padEnd(20)} —— 本轮未出现，跳过`);
    continue;
  }
  const missing = fields.filter((f) => !(f in sample.data));
  if (missing.length) {
    schemaOk = false;
    console.log(`      ${type.padEnd(20)} ❌ 缺字段: ${missing.join(', ')}`);
