...
function main() {
  ...
  if (contract !== null) {
    for (const L of WEB_LISTS) {
      ...
      /*
       * ★ 反方向（daemon 发得出、而这份表没跟上）——**每一轮都打印，即使今天没欠账。**
       *
       * 上一版把它挂在 `behind.length > 0` 上，结果是：三份表今天恰好同集 ⇒
       * **这行字在 CI 日志里一次都没出现过**。一条只在出事那天才第一次露面的提示，
       * 等于要求读到它的人当场从零理解它 —— 而他手上正拿着一个待修的 bug。
       * 所以改成常驻：holds 的时候也说一句，它才是一个**可被预先读到**的约定。
       */
      const behind = [...codes.keys()].filter((c) => !got.includes(c));
      if (behind.length === 0) {
        console.log(`   ⓘ daemon 发得出的码，这份表都跟上了（反方向刻意不判红，理由见下）`);
      } else {
        console.log(`   ⓘ daemon 发得出、而这份表没跟上的码：${behind.join(' / ')}`);
        console.log(`     ⇒ 用户在这些状态上会看到兜底话术，而不是这份表本该给的那句话。`);
      }
    }
    /*
     * ## 🔴 为什么反方向**刻意不判红**（Manager 2026-08-13 裁决）
     *
     * 「为什么不红」和判据本身一样重要 —— 不写下来，下一个读到那行 ⓘ 的人
     * 会觉得「这明明该红啊」，然后顺手改成红。（与 `:299` 那处刻意不补兜底同一种情况。）
     *
     * 理由是**兜底本身是诚实的**：`jobBlocked.UNKNOWN` 说的是「任务被挂起了」——
     * **那句话没说谎，只是没说全**。而这一整批门判红的标准是
     * **「说了它并不知道的事」**，不是「说得不够细」。
     *
     * 改成红的后果是**代价不对称**：daemon 修一个阻塞态的 bug，会被前端文案卡住 ——
     * 等于让一个诚实但笼统的兜底，去阻塞一个可能更要紧的修复。
     *
     * 所以这一行的作用是**让下一个人知道自己欠了一句话，而不是逼他当场还**。
     */
    console.log(
      '   （反方向不判红：兜底话术「任务被挂起了」是**诚实但笼统**，不是假话；\n' +
        '     判红会让 daemon 的修复被前端文案卡住 —— 代价不对称。欠着的那句话记在这里，不催。）',
    );
  }
