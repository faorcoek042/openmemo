 ...
 *
 * ★ 2026-08-23 从 60 收紧到 70。60 **太松，接不住一次已经发生过的漏跑**：
 *
 *   `[实测 run 32656407764]` win32 上 `61 passed, 8 failed`，而 linux/darwin
 *   都是 `72 passed, 0 failed`。少掉的 3 条是 **C1/C2/C3** —— CRLF 下 C0 拿不到
 *   "退回旧形状"的文本（`revertAttestGate` 返回 `null`），那三条被 `if` 整段跳过，
 *   **一声不吭**。而 C1 是整个凭证模拟器对着真实事故 run `31568740737` 的
 *   **唯一一处标定**：没有它，A2「审计绿 + 变异红 ⇒ 不发凭证」就只是一条自说自话。
 *
 *   61 > 60，所以这道下限**当时没有响**。一道专门防空转的守卫，自己放过了一次空转。
 *
 * 取 70 而不是 72：给正常增删留两条余量，但不给"整组消失"留余地
 * （C 组 17 条、D 组 29 条，任何一组塌掉都会跌破 70）。
 * CRLF 那个根因已由仓库根的 `.gitattributes` 修掉。
 */
const FLOOR = 70;
if (checked < FLOOR) {
