...
/** 从 git 索引里真删掉一个文件（模拟"从陈旧索引提交"的效果）。 */
function deleteTracked(root, file) {
  execFileSync('git', ['rm', '-q', '--cached', file], { cwd: root });
...
check('未跟踪（没 git add）的半成品测试不算数 —— 否则别人的草稿会让门禁红', () => {
  const s = sandbox();
  writeFileSync(join(s.root, 'packages/x/src/draft.test.ts'), '// 还没 git add\n');
  const r = run(s.root);
  if (r.status !== 0) throw new Error(`未跟踪文件让门禁红了：\n${r.all}`);
});
