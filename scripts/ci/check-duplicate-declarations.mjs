 ...
 * （⚠️ 下表是**校准记录**，量于 2026-09 收敛**之前**，不是今天的现状 ——
 * 现状由 CLI 每轮现算并打印。写死的数字会在它被量出来的下一刻开始过期）：
 *
 * | 档 | 当时跨包组数 | 其中真的是同一个概念 | 结论 |
 * |---|---|---|---|
...
if (isDirectRun(import.meta.url, process.argv[1])) {
   ...
   * ⚠️ 组数用**现算**的，校准数字才写死。
   *
   * 这两行原来把「5 组」「11 组」当成事实印出来 —— 那是**收敛之前**量的数，
   * 收敛之后当场就变成了假话（16_000 那组没了 ⇒ 4 组）。
   * 「一句写死的实测数，在它被量出来的下一刻就开始过期」正是本轮在清的那一族，
   * 这道门自己的输出不许犯。⇒ **现状用变量，校准结论标日期。**
   */
  console.log(
    `   · 裸数值相同：现在 ${dupValues.num.length} 组跨包。` +
      `【校准 2026-09，收敛之前】那时 5 组里**真的是同一个概念的只有 1 组**：` +
      `OMPK_VERSION / PLAN_VERSION / ASR_CHANNELS 都等于 1，` +
      `DEFAULT_SAMPLES_PER_PIXEL 与 SSE_REPLAY_BUFFER_SIZE 都等于 256。` +
      `唯一真的那组（16_000，四处名字各不相同）**已经收敛掉了** —— ` +
      `也就是说这一档今天一条真阳都没有，别把它读成"这道门在管数值常量"。`,
  );
  for (const g of dupValues.num) console.log(`     [${g.detail}] ${g.sites.join('  |  ')}`);
  console.log(
    `   · interface 字段名集合相同：现在 ${dupValues.fields.length} 组跨包。` +
      `【校准 2026-09，收敛之前】11 组里约 7 组是真的 —— 误报率低于上一档，` +
      `但仍不足以判红（\`Mirror\`/\`DownloadSource\`/\`ProbeTarget\` 那种 3 字段撞车会进来）。`,
  );
