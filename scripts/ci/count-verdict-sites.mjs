 ...
 * 也**不该**为它加减法 —— 那会把它减成负的。
 *
 * ## 🔴 为什么**方法简写也必须排掉**（2026-09-06 第四轮）
 *
 * 上一版把这条列成"已知残留偏差，反正本仓今天没有"。**那个判断的方向是错的**：
 *
 *   > 输出声称自己是 `≥N`（下界），只用来发现 0。而**定义处被多算 ⇒ 数偏高
 *   > ⇒ 那个 `≥` 是假的**。而且假的方式恰好落在防线看不见的那一侧：
 *   > 一个真实判据处为 **0** 的脚本，只要它自带一处定义，就会被报成 1，
 *   > **"数出 0 就出声"那条防线永远不会响。**
 *
 * 偏低不要紧（`≥` 仍然成立，最坏是多报一次"0"让人看一眼）；**偏高才是致命的**，
 * 因为它悄悄关掉了这个脚本唯一的那条判据。所以这里把所有**已知的**定义写法都排掉，
 * 让 `≥` 真的成立，而不是留一个说谎的 `≥`。
 *
 * 已排掉的定义写法：
 *   · `function name(` / `async function name(`
 *   · 类/对象里的方法简写 `name(a, b) {`（含 `async` / `get` / `set` 前缀）
 *   · 箭头式 `const name = (…) =>` —— **本来就匹配不上**（`name` 后面是 ` = (`，
 *     中间那个 `=` 让 `\s*\(` 失配），所以不需要处理，**更不该为它加减法**。
 *
 * ⚠️ 方法简写那条正则**必须限定在一行内、且括号里不许再有括号**
 *    （`[^()\n]*`）。我第一版写成 `[^)]*`，它跨行吃进了
 *    `selftest-duplicate-declarations.mjs` 里一个**多行 `assert(` 调用**
 *    （模板串里正好有个 `{`），于是报出一个根本不存在的"方法简写定义"。
 *    **差点据此写下一条假事实。**
 *
 * ⚠️ 仍然可能偏高的残余：参数列表跨行的方法简写、以及本文件没想到的定义写法。
 *    真出现了 `≥` 又会说谎 —— 所以 `selftest-verdict-sites.mjs` 的 E 组
 *    对**每一种**定义写法都钉了一条「只有定义、零调用 ⇒ 必须报 0」。
 */
export const countCalls = (code, name) => {
  /*
   * 方法简写没有关键字前缀，认不出来只能靠"它长什么样"：
   * 行首（只允许缩进）+ 名字 + 一对**不含括号也不跨行**的圆括号 + `{`。
   */
  const methodDefStarts = new Set();
  const methodRe = new RegExp(
    `^[ \\t]*(?:async[ \\t]+|get[ \\t]+|set[ \\t]+)?${name}[ \\t]*\\([^()\\n]*\\)[ \\t]*\\{`,
    'gm',
  );
  for (const m of code.matchAll(methodRe)) methodDefStarts.add(m.index + m[0].lastIndexOf(name));

  const re = new RegExp(`(^|[^A-Za-z0-9_.$])(async\\s+)?(function\\s+)?${name}\\s*\\(`, 'gm');
  let n = 0;
  for (const m of code.matchAll(re)) {
    if (m[3] !== undefined) continue; // `function name(` —— 定义
    if (methodDefStarts.has(m.index + m[0].lastIndexOf(name))) continue; // 方法简写 —— 定义
    n++;
  }
  return n;
};
 ...
 *
 * ## ⚠️ 「下界」这三个字是有代价的 —— 两个方向**不对称**
 *
 * ③ 是这个脚本唯一的判据，而它**只在数偏低时仍然安全**：
 *
 *   · 偏低 ⇒ `≥` 仍然成立；最坏是多报一次「0」，让人白看一眼。**无害。**
 *   · 偏高 ⇒ `≥` 是**假的**，而且假在防线看不见的那一侧：一个真实判据处为 **0**
 *     的守卫，只要自带一处定义就会被报成 1，**③ 那条告警永远不会响**。
 *
 * 所以凡是会让数**偏高**的东西都必须被消灭，而不是记成"已知残留偏差"。
 * `countCalls()` 上面那段列着已排掉的全部定义写法；
 * `selftest-verdict-sites.mjs` 的 **E 组**对**每一种**都钉了一条
 * 「只有定义、零调用 ⇒ 必须报 0」—— 少钉一种，就等于给 ③ 开一个只进不出的洞。
 *
 * ⚠️ 想过用 AST 一劳永逸（`leg-coverage` 那条"按结构判、不要名字清单"的思路）。
 *    **做不到**：本仓 `package.json` 没有任何 JS 解析器依赖，`node_modules` 里也没有；
 *    我这台机器上 `require('acorn')` 能过，是因为它解析到了**系统的 Debian 包**
 *    `/usr/share/nodejs/acorn` —— CI runner 上没有那份。照着用会本地绿、CI 红。
 *    真要走 AST，得先往 `package.json` 里加一个依赖，那是另一个决定。
 */
