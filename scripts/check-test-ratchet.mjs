 ...
 * - 树上出现**基线里没有的**测试文件 → 也红，提示跑 `--update`。
 *   这条不是洁癖：基线**不自动跟上新增，就会永远落后于最危险的那一档**
 *   ——「刚加进来、还没被任何人记住」的文件，正是这次事故删掉的那一种。
 *
...
if (unregistered.length > 0) {
  problems.push('unregistered');
  console.error(`\n✘ **有 ${unregistered.length} 个测试文件还没登记进基线**：\n`);
  for (const f of unregistered) console.error(`   + ${f}`);
  console.error(
    [
      '',
      '  修法就一条命令（**只增不减，闭眼跑也不会掩盖删除**）：',
      '',
      '      pnpm check:test-ratchet --update',
      '',
      '  然后把 scripts/test-ratchet-baseline.json 一起提交。',
      '',
      '  为什么新增也要红：基线不跟上新增，就永远保护不到**最危险的那一档** ——',
      '  「刚加进来、还没被任何人记住」的文件。这次事故删掉的正是这一种。',
    ].join('\n'),
  );
}
