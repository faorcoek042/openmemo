...
const RULES = [
  ...
  },
  {
    ...
    why:
      ...
      '13 条散文里 8 条被判成"上游提供"，**8 条全部误判**（这个数由 ' +
      '`sha256ProvenanceRegex.test.ts` 重放那条正则算出来，不是抄的常量），' +
  ...
  },
  {
    ...
    dir: 'apps/web/src/features/components',
    why:
      '「关于这一份制品，有件事我们得先说」——今天挂着的是 4 条组件承认' +
      '**端到端从没在那个平台上跑过**（`whispercpp-cpu-linux-x64` / `-cpu-win-x64` / ' +
      '`-vulkan-win-x64` / `-metal-macos-arm64`）。这一格是**审计实测抓出来才有的**：' +
      '那句承认原本只活在 `sha256Provenance` 那段长文里，长文一折进「这一份是怎么来的」，' +
      '承认就跟着不见了，卡片正面只剩一个干净的「安装 v1.9.1」。' +
      '零消费 ⇒ 回到那个现场：**「我们不知道」被收回成沉默**，而那是这几版的第一条红线。',
  },
];
